...
- TOKENMAXX stewardship_common second-pass pins after #65: expand self-tests to
  1540 cases (was 1468) fail-closing live stewardship_common integrity —
  `ROOT` exact / `DANGEROUS_LINK_SCHEMES` exact / `FENCED_BLOCK_RE` exact /
  ...
  `is_file` / `label or str` / no invent-product surface, plus
  stewardship_common gate contract pins in `check_badge_standard.py` —
  stewardship_common second-pass slice only (not badge / wiki / relative /
  schema / CI workflow pin / fixture-reject spam); no invent-product; no
  fourth badge
