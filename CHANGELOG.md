...
- TOKENMAXX badge-standard gate pins after #48: expand self-tests to 1188
  cases (was 1114) fail-closing live badge-row integrity — `REQUIRED_ORDER` /
  `MAX_BADGES=3` / `EXPECTED_REPO` / `BADGE_LINE_RE` / contiguous row /
  shields license / absolute https workflow links / invent-product
  `FORBIDDEN_BADGE_HINTS` + `SECRET_URL_HINTS` /
  `verify_badge_standard_source_contract` in `stewardship_common` — badge-standard
  gate contract slice only (not schema / wiki-outline / relative-link /
  stewardship_common / CI workflow pin / fixture-reject spam); no invent-product;
  no fourth badge
