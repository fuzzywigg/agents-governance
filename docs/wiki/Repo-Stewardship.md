...
`cancel-in-progress:` / `permissions:` present / reject `actions`|
`packages`|`pull-requests: write` / `finditer` uses / docker continue /
`rsplit[-1]` / third-pass docstring,
plus stewardship_common third-pass after #111: future annotations /
import re+Path / exact PRIVATE KEY+gh*+github_pat+sk|rk / tuple typing /
`FENCED_BLOCK_RE.sub` / label|relative_to / pattern.search / lowered /
re.escape / URL-ish / ROOT.glob+update / set[Path] / workflows join /
for-loops / Shared helpers / endswith(=) / MEMORY dumps /
str|None+list[Path] / FORBIDDEN head / SECRET_URL token-prefix members
— lands closed #112 leftover).
