...
plus run_stewardship runner pins after #117: shebang / `set -euo pipefail` /
dirname+cd ROOT / same-set-as-CI / `python3 scripts/<gate>` /
badge→wiki→schema→relative / pwd + Run-all commentary /
`check_run_stewardship_gate_contract` — lands closed #122/#96 leftover).
