...
Fail-closed wiki/mdlink leftover residual after #278 tip (beyond #252 saturated
after_243 pins; lands closed #281/#277 leftover; NOT lychee/blob-503 leftover #278 /
...
# Fail-closed after #227: markdown-lint/link-check residual exact layouts
    ...
    # Wiki/mdlink leftover residual after #278 tip: md/link residual harden beyond #252.
    link_uses_adj = (
        "      - name: Check links\n"
    ...
    if link_uses_adj not in link:
        fail(
            ...
            "(wiki/mdlink leftover residual after #278)",
            errors,
        )
    lint_uses_adj = (
        "      - name: Run markdownlint\n"
...
def check_badge_standard_gate_contract(errors: list[str]) -> None:
    ...
    leftover_278_doc = "wiki/mdlink leftover residual after " + "#278"
    if leftover_278_doc not in text:
        fail(
            "check_badge_standard.py docstring must keep " + leftover_278_doc + " pin",
            errors,
        )
...
def check_wiki_outline_gate_contract(errors: list[str]) -> None:
    ...
    leftover_278_wiki = "Wiki/mdlink leftover residual after " + "#278"
    if leftover_278_wiki not in wiki_text:
        fail(
            "check_wiki_outline.py docstring must keep " + leftover_278_wiki + " pin",
            errors,
        )
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    # Fail-closed after #278: markdown link residual harden (wiki/mdlink leftover
    # residual; lands #281/#277; NOT lychee/blob-503 leftover #278 /
    ...
    leftover_278_rel = "Wiki/mdlink leftover residual after " + "#278"
    if leftover_278_rel not in text:
        fail(
            "check_relative_links.py docstring must keep " + leftover_278_rel + " pin",
            errors,
        )
