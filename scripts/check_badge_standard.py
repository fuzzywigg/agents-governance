...
- schema leftover residual after #272: leftover residual helper needles + leftover residual status/surface stubs (schema leftover residual slice)
  (NOT Pass-2 residual + template leftover #272 / NOT path-filter/path-order leftover #262 / NOT schema leftover #258 / NOT wiki/mdlink leftover #252 / NOT path-edges residual #244 / NOT wiki outline/PUBLISH #243 / NOT md/link residual #239 / NOT schema residual (pass-5) #233)
- common leftover after #252: elif hint in text / for pattern in globs /
  found: set[Path] = set() / if not path.is_file(): (load_workflow)
- common leftover residual after #272: errors.append(msg) / lowered = target.strip().lower() /
...
def check_stewardship_common_contract(errors: list[str]) -> None:
    """Fail-close live stewardship_common wiring (after #111; deepen after #65/#46; leftover after #252; leftover residual after #272)."""
    ...
    # Fail-closed after #272: common leftover residual helper / constant / needle pins
    # (common leftover residual slice only; not Pass-2 residual + template leftover #272 /
    ...
    # Split literals so self-mutation of contiguous names cannot neutralize checks.
    common_leftover_272_doc = "Residual leftover after " + "#272 (common leftover residual"
    if common_leftover_272_doc not in text:
        fail(
            "stewardship_common.py docstring must pin " + common_leftover_272_doc,
            errors,
        )
    not_pass2_common_272 = "NOT Pass-2 residual + template leftover " + "#272"
    if not_pass2_common_272 not in text:
        fail(
            "stewardship_common.py docstring must pin " + not_pass2_common_272,
            errors,
        )
    ...
    common_host_272 = "common leftover residual after " + "#272"
    self_text_common_272 = BADGE_GATE.read_text(encoding="utf-8")
    if common_host_272 not in self_text_common_272:
        fail(
            "check_badge_standard.py must keep " + common_host_272 + " host pin",
            errors,
        )
...
def check_stewardship_schema_gate_contract(errors: list[str]) -> None:
    """Fail-close live stewardship-schema gate wiring (after #72/#75; third-pass after #132; deepen after #189; policy-schema pass-4 after #208; schema residual after #225; leftover after #252; leftover residual after #272)."""
    ...
    # Fail-closed after #272: schema leftover residual helper / constant / needle pins
    # (schema leftover residual slice only; not Pass-2 residual + template leftover #272 /
    # not path-filter/path-order leftover #262 /
    # not schema leftover #258 / not wiki/mdlink leftover #252 /
    # not path-edges residual #244 / not wiki outline/PUBLISH #243 / not md/link residual #239 /
    # not schema residual (pass-5) #233 spam).
    # Split literals so self-mutation of contiguous names cannot neutralize checks.
    leftover_272_doc = "Residual leftover after " + "#272 (schema leftover residual"
    if leftover_272_doc not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + leftover_272_doc,
            errors,
        )
    not_pass2_272 = "NOT Pass-2 residual + template leftover " + "#272"
    ...
    leftover_host_272 = "schema leftover residual after " + "#272"
    self_text6 = BADGE_GATE.read_text(encoding="utf-8")
    if leftover_host_272 not in self_text6:
        fail(
            "check_badge_standard.py must keep " + leftover_host_272 + " host pin",
            errors,
        )
