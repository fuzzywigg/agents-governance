...
Fail-closed wiki/mdlink leftover residual after #337 tip (beyond #326 saturated
after_326 wiki pins; NOT Pass-2 residual / templates leftover #337 /
...
# Fail-closed after #227: markdown-lint/link-check residual exact layouts
    ...
    # Wiki/mdlink leftover residual after #337 tip: md/link residual harden beyond #326
    ...
    if link_verbose_noprogress not in link:
        fail(
            ...
            "(wiki/mdlink leftover residual after #337)",
            errors,
        )
    link_token_exclude = (
        "            --github-token ${{ secrets.GITHUB_TOKEN }}\n"
...
def check_badge_standard_gate_contract(errors: list[str]) -> None:
    ...
    leftover_337_doc = "wiki/mdlink leftover residual after " + "#337"
    if leftover_337_doc not in text:
        fail(
            "check_badge_standard.py docstring must keep " + leftover_337_doc + " pin",
            errors,
        )
...
def check_wiki_outline_gate_contract(errors: list[str]) -> None:
    ...
    leftover_337_wiki = "Wiki/mdlink leftover residual after " + "#337"
    if leftover_337_wiki not in wiki_text:
        fail(
            "check_wiki_outline.py docstring must keep " + leftover_337_wiki + " pin",
            errors,
        )
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    leftover_337_rel = "Wiki/mdlink leftover residual after " + "#337"
    if leftover_337_rel not in text:
        fail(
            "check_relative_links.py docstring must keep " + leftover_337_rel + " pin",
            errors,
        )
