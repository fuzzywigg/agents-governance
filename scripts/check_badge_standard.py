...
Fail-closed actionlint path-filter/path-order residual leftover deepen after #297
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    Path-filter/path-order residual leftover deepen after #297 (NOT schema residual CI
    #297 / NOT wiki/mdlink leftover residual #293 / NOT stewardship-schema residual #282 /
    NOT lychee/blob-503 harden #278 / NOT Pass-2 residual leftover #272 /
    NOT saturated leftover #262 /
    NOT stewardship-schema leftover #258 /
    NOT wiki/mdlink leftover #252 / NOT saturated residual #244 /
    ...
    # Path-filter/path-order residual leftover deepen after #297 (DISTINCT leftover;
    # NOT schema residual CI #297 / NOT wiki/mdlink leftover residual #293 /
    # NOT stewardship-schema residual #282 /
    # NOT lychee/blob-503 harden #278 / NOT Pass-2 residual leftover #272 /
    # NOT saturated leftover #262 /
    ...
    for wf_name, body in (
        ...
        if dispatch_concurrency not in body:
            fail(
                ...
                "adjacency "
                "(path-filter/path-order residual leftover deepen after #297)",
                errors,
            )
    ...
    setup_name_297 = "name: Set up Python"
    setup_idx_297 = stew.find(setup_name_297)
    install_idx_297 = stew.find("name: Install PyYAML (schema parser)")
    gates_name_297 = (
        "name: Stewardship gates (badge / wiki / schema / relative links)"
    )
    gates_idx_297 = stew.find(gates_name_297)
    self_tests_idx_297 = stew.find("name: Stewardship gate self-tests")
    dl_idx_297 = stew.find("name: Download actionlint")
    run_idx_297 = stew.find("name: actionlint existing workflow paths")
    if (
        setup_idx_297 < 0
        or install_idx_297 < 0
        or gates_idx_297 < 0
        or self_tests_idx_297 < 0
        or dl_idx_297 < 0
        or run_idx_297 < 0
        or not (
            setup_idx_297
...
def check_workflow_hardening_gate_contract(errors: list[str]) -> None:
    ...
    # Path-filter/path-order residual leftover deepen after #297 (DISTINCT leftover).
    path_leftover_297_doc = (
        "Path-filter/path-order residual leftover deepen after " + "#297"
    )
    if path_leftover_297_doc not in text:
        fail(
            "check_workflow_hardening docstring must pin " + path_leftover_297_doc,
            errors,
        )
    path_module_leftover_297 = (
        "path-filter/path-order residual leftover deepen after " + "#297"
    )
    if path_module_leftover_297 not in text:
        fail(
            "check_badge_standard.py module docstring must pin "
            + path_module_leftover_297,
            errors,
        )
    not_schema_residual_ci_297 = "NOT schema residual CI " + "#297"
    if not_schema_residual_ci_297 not in text:
        fail(
            "path leftover-after-297 must keep "
            + not_schema_residual_ci_297
            + " distinctness pin",
            errors,
        )
    ...
    path_leftover_297_pins = (
        ...
        (
            "path-filter/path-order residual leftover deepen after " + "#297",
            "path leftover-after-297 wording pin",
        ),
    )
    for needle, label in path_leftover_297_pins:
        if needle not in text:
            fail(
                "check_workflow_hardening must keep " + label,
                errors,
...
def check_stewardship_common_contract(errors: list[str]) -> None:
    """Fail-close live stewardship_common wiring (after #111; deepen after #65/#46; leftover after #252; leftover residual after #262; residual CI deepen after #293)."""
...
def check_stewardship_schema_gate_contract(errors: list[str]) -> None:
    """Fail-close live stewardship-schema gate wiring (after #72/#75; third-pass after #132; deepen after #189; policy-schema pass-4 after #208; schema residual after #225; leftover after #252; leftover residual after #262; residual CI deepen after #293)."""
