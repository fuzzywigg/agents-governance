...
Fail-closed wiki/mdlink leftover residual after #314 tip (beyond #293 saturated
after_293 pins; lands #318/#308/#307/#302 leftover; NOT path-filter/path-order leftover #314 /
...
# Fail-closed after #227: markdown-lint/link-check residual exact layouts
    ...
    # Wiki/mdlink leftover residual after #314 tip: md/link residual harden beyond #293 (lands #318/#308/#307/#302).
    link_token_args_adj = (
        "          token: ${{ secrets.GITHUB_TOKEN }}\n"
        "          args: >-"
    )
    if link_token_args_adj not in link:
        fail(
            ...
            "(wiki/mdlink leftover residual after #314)",
            errors,
        )
    link_steps_triple = (
        "      - uses: actions/checkout@v7\n"
    ...
    if lint_steps_triple not in lint:
        fail(
            ...
            "triple adjacency (wiki/mdlink leftover residual after #314)",
            errors,
        )
...
def check_badge_standard_gate_contract(errors: list[str]) -> None:
    ...
    leftover_314_doc = "wiki/mdlink leftover residual after " + "#314"
    if leftover_314_doc not in text:
        fail(
            "check_badge_standard.py docstring must keep " + leftover_314_doc + " pin",
            errors,
        )
...
def check_wiki_outline_gate_contract(errors: list[str]) -> None:
    ...
    leftover_314_wiki = "Wiki/mdlink leftover residual after " + "#314"
    if leftover_314_wiki not in wiki_text:
        fail(
            "check_wiki_outline.py docstring must keep " + leftover_314_wiki + " pin",
            errors,
        )
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    leftover_314_rel = "Wiki/mdlink leftover residual after " + "#314"
    if leftover_314_rel not in text:
        fail(
            "check_relative_links.py docstring must keep " + leftover_314_rel + " pin",
            errors,
        )
