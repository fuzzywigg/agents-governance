...
Fail-closed run_stewardship runner pins (live path after #117; deepen after #165; lands closed #140; lands closed #96):
...
- Pass-2 after #165: exact ROOT="$(cd "$(dirname "$0")/.." && pwd)" /
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    # Fail-closed after #165: runner before self-tests (CI reliability leftover).
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    if not RUN_STEWARDSHIP.is_file():
        ...
        # Fail-closed after #165: exactly four python3 scripts/ gate lines.
        ...
        # Fail-closed after #165: no soft-fail softening of runner posture.
        ...
        # Fail-closed after #165: parent-dir fragment from live ROOT assign.
        ...
        # Fail-closed after #165: locally commentary (same set as CI path).
        ...
        # Fail-closed after #165: exact ROOT assign (CI reliability leftover).
...
def check_run_stewardship_gate_contract(errors: list[str]) -> None:
    """Fail-close live run_stewardship_checks.sh wiring (after #117/#165; not lychee/mdlint spam)."""
    if not BADGE_GATE.is_file():
    ...
    # Fail-closed after #165 deepen: exact ROOT / four-gate / soft-fail / CI order.
    live_165 = "Fail-closed after " + "#165"
    if live_165 not in text:
        fail(
            "run_stewardship pins must keep " + live_165 + " marker",
            errors,
        )
