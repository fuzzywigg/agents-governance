...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    setup_name_299 = "name: Set up Python"
    setup_idx_299 = stew.find(setup_name_299)
    install_idx_299 = stew.find("name: Install PyYAML (schema parser)")
    gates_name_299 = (
        "name: Stewardship gates (badge / wiki / schema / relative links)"
    )
    gates_idx_299 = stew.find(gates_name_299)
    self_tests_idx_299 = stew.find("name: Stewardship gate self-tests")
    dl_idx_299 = stew.find("name: Download actionlint")
    run_idx_299 = stew.find("name: actionlint existing workflow paths")
    if (
        setup_idx_299 < 0
        or install_idx_299 < 0
        or gates_idx_299 < 0
        or self_tests_idx_299 < 0
        or dl_idx_299 < 0
        or run_idx_299 < 0
        or not (
            setup_idx_299
...
def check_workflow_hardening_gate_contract(errors: list[str]) -> None:
    ...
    for needle, label in path_leftover_299_pins:
        if needle not in text:
            fail(
                "check_workflow_hardening must keep " + label,
                errors,
