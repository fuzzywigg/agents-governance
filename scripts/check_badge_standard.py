...
Fail-closed actionlint-style pins (live path after #75; reopen #63 intent):
- top-level name: / on: / jobs: / jobs.*.runs-on / jobs.*.steps / timeout-minutes
- no pull_request_target / no permissions: write-all / no contents: write
- no id-token: write / no packages: write / no actions: write / no pull-requests: write
- actions must be @-pinned (not main|master|latest); docker:// uses skipped
- unpinned uses rejected; gate contract fail-closes helper/needle drift
"""
...
def check_actionlint_style(errors: list[str]) -> None:
    ...
    Live fail-closed pins after #75 (not CI workflow / badge-row spam):
    top-level name: / on: / jobs: / jobs.*.runs-on / jobs.*.steps / timeout-minutes;
    reject pull_request_target / permissions: write-all / contents: write /
    id-token: write / packages: write / actions: write / pull-requests: write;
    @-pin actions (not main|master|latest); skip docker://; reject unpinned uses.
    """
    for name in REQUIRED_WORKFLOWS:
        ...
        # Fail-closed after #75: every workflow must declare an on: trigger block.
        if not re.search(r"(?m)^on:\s*$", text) and not re.search(r"(?m)^on:\s*\S", text):
            fail(f"{name}: actionlint-style requires top-level on:", errors)
        # Fail-closed after #75: every workflow must declare a jobs: map.
        if not re.search(r"(?m)^jobs:\s*$", text) and "jobs:" not in text:
            fail(f"{name}: actionlint-style requires top-level jobs:", errors)
...
def check_actionlint_style_gate_contract(errors: list[str]) -> None:
    """Fail-close live actionlint-style gate wiring (after #75; not CI workflow spam)."""
    if not BADGE_GATE.is_file():
        fail("Missing scripts/check_badge_standard.py (actionlint-style host)", errors)
        return
    text = BADGE_GATE.read_text(encoding="utf-8")
    # Split pin literals so self-mutation of contiguous names cannot neutralize checks.
    host_pin = "actionlint-style " + "host"
    if host_pin not in text:
        fail(
            "check_actionlint_style_gate_contract must keep " + host_pin + " pin",
            errors,
        )
    spam_pin = "not CI workflow " + "spam"
    if spam_pin not in text:
        fail(
            "check_actionlint_style_gate_contract must keep " + spam_pin + " pin",
            errors,
        )
    ...
    contract_fn = "check_actionlint_style_" + "gate_contract"
    if f"def {contract_fn}(" not in text:
        fail(
            "check_badge_standard.py must provide " + contract_fn + "()",
            errors,
    ...
    on_pin = "top-level " + "on:"
    if on_pin not in text:
        fail(
            "check_actionlint_style must require " + on_pin,
            errors,
        )
    jobs_top_pin = "top-level " + "jobs:"
    if jobs_top_pin not in text:
        fail(
            "check_actionlint_style must require " + jobs_top_pin,
            errors,
        )
    ...
    packages_write_pin = "packages: " + "write"
    if packages_write_pin not in text:
        fail(
            "check_actionlint_style must reject " + packages_write_pin,
            errors,
        )
    actions_write_pin = "actions: " + "write"
    if actions_write_pin not in text:
        fail(
            "check_actionlint_style must reject " + actions_write_pin,
            errors,
        )
    pr_write_pin = "pull-requests: " + "write"
    if pr_write_pin not in text:
        fail(
            "check_actionlint_style must reject " + pr_write_pin,
            errors,
        )
    docker_pin = "docker:" + "//"
    ...
    uses_re_pin = r"(?m)^\s*-\s*" + r"uses:\s*([^\s#]+)"
    if uses_re_pin not in text:
        fail(
            "check_actionlint_style must scan uses: via actionlint-style regex",
            errors,
        )
