...
- Markdown-lint/link-check workflow edges after #199 (lands closed #201/#192;
  NOT Pass-2 residual #199 / NOT schema fourth-pass #204 / NOT path-order #189 /
  NOT schema third-pass #191): args: >- /
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    # Fail-closed after #199: markdown-lint / link-check workflow edges
    # (distinct from Pass-2 residual #199 / schema #191/#204 / path-order #189 /
    # wiki-index #181 / path-filter #176 spam).
    if "args: >-" not in link:
        fail(
...
def check_workflow_hardening_gate_contract(errors: list[str]) -> None:
    ...
    # Fail-closed after #199: markdown-lint / link-check workflow edge pins
    # (not path-filter / Pass-2 / wiki-index / schema / path-order spam).
    edge_doc = "Markdown-lint/link-check workflow edges after " + "#199"
    if edge_doc not in text:
        fail(
            "check_workflow_hardening docstring must pin " + edge_doc,
            errors,
        )
    module_edge = "markdown-lint/link-check edges after " + "#199"
