...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    if not RUN_STEWARDSHIP.is_file():
        ...
        for gate in (
            ...
            # Fail-closed after #90: each gate runs via python3 scripts/ prefix.
        ...
        # Fail-closed after #90: live runner shell posture (docs CI local path).
...
def check_run_stewardship_gate_contract(errors: list[str]) -> None:
    """Fail-close live run_stewardship_checks.sh wiring (after #90; not lychee/mdlint spam)."""
    if not BADGE_GATE.is_file():
...
def main() -> int:
    ...
    # Fail-closed after #90: run_stewardship runner contract early.
