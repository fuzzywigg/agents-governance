...
Fail-closed actionlint path-filter/path-order residual leftover deepen after #262
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    Path-filter/path-order residual leftover deepen after #262 (NOT saturated
    leftover #262 / NOT stewardship-schema leftover #258 /
    NOT wiki/mdlink leftover #252 / NOT saturated residual #244 /
    ...
    # Path-filter/path-order residual leftover deepen after #262 (DISTINCT leftover;
    # NOT saturated leftover #262 / NOT stewardship-schema leftover #258 /
    # NOT wiki/mdlink leftover #252 / NOT saturated residual #244 /
    # NOT saturated deepen #225/#203 / NOT #189 path-order / NOT #176 layouts /
    # NOT schema #191/#216 / NOT Pass-2 residual #199/#203 /
    ...
    for wf_name, body in (
        ...
        if dispatch_concurrency not in body:
            fail(
                ...
                "adjacency "
                "(path-filter/path-order residual leftover deepen after #262)",
                errors,
            )
    ...
    setup_name_262 = "name: Set up Python"
    setup_idx_262 = stew.find(setup_name_262)
    install_idx_262 = stew.find("name: Install PyYAML (schema parser)")
    gates_name_262 = (
        "name: Stewardship gates (badge / wiki / schema / relative links)"
    )
    gates_idx_262 = stew.find(gates_name_262)
    self_tests_idx_262 = stew.find("name: Stewardship gate self-tests")
    dl_idx_262 = stew.find("name: Download actionlint")
    run_idx_262 = stew.find("name: actionlint existing workflow paths")
    if (
        setup_idx_262 < 0
        or install_idx_262 < 0
        or gates_idx_262 < 0
        or self_tests_idx_262 < 0
        or dl_idx_262 < 0
        or run_idx_262 < 0
        or not (
            setup_idx_262
...
def check_workflow_hardening_gate_contract(errors: list[str]) -> None:
    ...
    # Path-filter/path-order residual leftover deepen after #262 (DISTINCT leftover).
    path_leftover_262_doc = (
        "Path-filter/path-order residual leftover deepen after " + "#262"
    )
    if path_leftover_262_doc not in text:
        fail(
            "check_workflow_hardening docstring must pin " + path_leftover_262_doc,
            errors,
        )
    path_module_leftover_262 = (
        "path-filter/path-order residual leftover deepen after " + "#262"
    )
    if path_module_leftover_262 not in text:
        fail(
            "check_badge_standard.py module docstring must pin "
            + path_module_leftover_262,
            errors,
    ...
    not_schema_leftover_258_262 = "NOT stewardship-schema leftover " + "#258"
    if not_schema_leftover_258_262 not in text:
        fail(
            "path leftover-after-262 must keep "
            + not_schema_leftover_258_262
            + " distinctness pin",
            errors,
        )
    not_wiki_mdlink_252_262 = "NOT wiki/mdlink leftover " + "#252"
    if not_wiki_mdlink_252_262 not in text:
        fail(
            "path leftover-after-262 must keep "
            + not_wiki_mdlink_252_262
            + " distinctness pin",
            errors,
        )
    not_saturated_residual_244_262 = "NOT saturated residual " + "#244"
    if not_saturated_residual_244_262 not in text:
        fail(
            "path leftover-after-262 must keep "
            + not_saturated_residual_244_262
            + " distinctness pin",
            errors,
        )
    not_wiki_publish_243_262 = "NOT wiki outline/PUBLISH leftover " + "#243"
    if not_wiki_publish_243_262 not in text:
        fail(
            "path leftover-after-262 must keep "
            + not_wiki_publish_243_262
            + " distinctness pin",
            errors,
        )
    not_mdlink_239_262 = "NOT md/link residual " + "#239"
    if not_mdlink_239_262 not in text:
        fail(
            "path leftover-after-262 must keep "
            + not_mdlink_239_262
            + " distinctness pin",
    ...
    not_stew_schema_233_262 = (
        "NOT stewardship-checks/schema leftover " + "#233"
    )
    if not_stew_schema_233_262 not in text:
        fail(
            "path leftover-after-262 must keep "
            + not_stew_schema_233_262
            + " distinctness pin",
    ...
    not_wiki_badge_227_262 = "NOT wiki-badge leftover " + "#227"
    if not_wiki_badge_227_262 not in text:
        fail(
            "path leftover-after-262 must keep "
            + not_wiki_badge_227_262
            + " distinctness pin",
            errors,
        )
    not_saturated_deepen_225_262 = "NOT saturated deepen " + "#225"
    if not_saturated_deepen_225_262 not in text:
        fail(
            "path leftover-after-262 must keep "
            + not_saturated_deepen_225_262
            + " distinctness pin",
            errors,
        )
    not_pass2_220_leftover_262 = "NOT Pass-2 leftover + " + "md/link #220"
    if not_pass2_220_leftover_262 not in text:
        fail(
            "path leftover-after-262 must keep "
            + not_pass2_220_leftover_262
            + " distinctness pin",
            errors,
        )
    path_leftover_262_pins = (
        (
            "contiguous workflow_dispatch:/concurrency: " + "adjacency",
        ...
        (
            "path-filter/path-order residual leftover deepen after " + "#262",
            "path leftover-after-262 wording pin",
        ),
    )
    for needle, label in path_leftover_262_pins:
        if needle not in text:
            fail(
                "check_workflow_hardening must keep " + label,
                errors,
