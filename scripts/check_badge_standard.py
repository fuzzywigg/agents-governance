...
Fail-closed docs CI config pins (live path after #83; ECO-048):
...
def check_workflows_and_license(errors: list[str]) -> None:
    ...
    if not MARKDOWNLINT_CONFIG.is_file():
        ...
        # Fail-closed after #83: exact live MD013 object (docs CI config slice).
        if '"MD013": { "line_length": 200 }' not in md_cfg:
            fail(
                '.markdownlint.json must pin "MD013": { "line_length": 200 }',
                errors,
            )
        # Fail-closed after #83: exact live MD024 object (docs CI config slice).
...
def check_lycheeignore(errors: list[str]) -> None:
    ...
    # Fail-closed after #83: live tree pins the regex-escaped shields form.
...
def check_docs_ci_config_gate_contract(errors: list[str]) -> None:
    """Fail-close live docs CI config wiring (after #83; not CI workflow spam)."""
    if not BADGE_GATE.is_file():
        fail("Missing scripts/check_badge_standard.py (docs CI config host)", errors)
        return
    text = BADGE_GATE.read_text(encoding="utf-8")
    # Split pin literals so self-mutation of contiguous names cannot neutralize checks.
    host_pin = "docs CI config " + "host"
    if host_pin not in text:
        fail(
            "check_docs_ci_config_gate_contract must keep " + host_pin + " pin",
            errors,
        )
    spam_pin = "not CI workflow " + "spam"
    if spam_pin not in text:
        fail(
            "check_docs_ci_config_gate_contract must keep " + spam_pin + " pin",
            errors,
        )
    ...
    contract_fn = "check_docs_ci_config_" + "gate_contract"
    if f"def {contract_fn}(" not in text:
        fail(
            "check_badge_standard.py must provide " + contract_fn + "()",
            errors,
        )
    live_doc = "Live fail-closed pins after " + "#83"
    ...
    for rule in ("MD033", "MD041", "MD060"):
        # Gate uses re.search(r'"MD033"\s*:\s*false\b', md_cfg) form.
    ...
    docs_slice = "docs CI config " + "slice"
...
def main() -> int:
    ...
    # Fail-closed after #83: docs CI config contract early (lycheeignore +
    # markdownlint.json pins) so config-helper renames fail closed.
    check_docs_ci_config_gate_contract(errors)
    if errors:
