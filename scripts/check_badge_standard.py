...
def check_workflows_and_license(errors: list[str]) -> None:
    ...
    if not MARKDOWNLINT_CONFIG.is_file():
        ...
        # Fail-closed after #94: exact live MD013 object (docs-lint slice).
        if '"MD013": { "line_length": 200 }' not in md_cfg:
            fail(
                '.markdownlint.json must pin "MD013": { "line_length": 200 }',
                errors,
            )
        # Fail-closed after #94: exact live MD024 object (docs-lint slice).
...
def check_lycheeignore(errors: list[str]) -> None:
    ...
    Live fail-closed pins after #94 (docs-lint slice; not CI workflow spam):
    ...
    # Fail-closed after #94: live tree pins the regex-escaped shields form.
    ...
    # Fail-closed after #94: live excludes for known lychee false-positives.
    ...
    # Fail-closed after #94: commentary must keep stewardship license-badge posture.
    lowered = text.lower()
...
def check_docs_lint_gate_contract(errors: list[str]) -> None:
    """Fail-close live docs-lint wiring (after #94; not CI workflow spam)."""
    if not BADGE_GATE.is_file():
    ...
    live_doc = "Live fail-closed pins after " + "#94"
...
def main() -> int:
    ...
    # Fail-closed after #94: docs-lint contract early (lycheeignore + markdownlint).
