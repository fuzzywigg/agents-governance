...
Fail-closed actionlint path-filter/path-order residual leftover deepen after #244
(NOT saturated residual #244 / NOT wiki/mdlink leftover #252 /
NOT saturated deepen #225/#203 /
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    Path-filter/path-order residual leftover deepen after #244 (NOT saturated
    residual #244 / NOT wiki/mdlink leftover #252 /
    ...
    # Path-filter/path-order residual leftover deepen after #244 (DISTINCT leftover;
    ...
    for wf_name, body in (
        ...
        if not push_pr_re.search(body):
            fail(
                ...
                "adjacency "
                "(path-filter/path-order residual leftover deepen after #244)",
                errors,
            )
    ...
    gates_idx_244 = stew.find(gates_name_244)
    self_tests_idx_244 = stew.find("name: Stewardship gate self-tests")
    dl_idx_244 = stew.find("name: Download actionlint")
    run_idx_244 = stew.find("name: actionlint existing workflow paths")
    if (
        install_idx < 0
        or gates_idx_244 < 0
        or self_tests_idx_244 < 0
        or dl_idx_244 < 0
        or run_idx_244 < 0
        or not (
            install_idx
