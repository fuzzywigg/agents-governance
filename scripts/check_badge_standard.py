...
Fail-closed wiki/mdlink leftover residual after #272 tip (beyond #252 saturated
after_243 pins; lands closed #281/#277 leftover on post-#278 tip;
NOT stewardship-schema leftover residual #282 / NOT lychee/blob-503 leftover #278 / NOT Pass-2 residual + templates #272 /
...
# Fail-closed after #227: markdown-lint/link-check residual exact layouts
    ...
    # Wiki/mdlink leftover residual after #272 tip: md/link residual harden beyond #252.
    link_uses_adj = (
        "      - name: Check links\n"
    ...
    if link_uses_adj not in link:
        fail(
            ...
            "(wiki/mdlink leftover residual after #272)",
            errors,
        )
    lint_uses_adj = (
        "      - name: Run markdownlint\n"
...
def check_badge_standard_gate_contract(errors: list[str]) -> None:
    ...
    leftover_272_doc = "wiki/mdlink leftover residual after " + "#272"
    if leftover_272_doc not in text:
        fail(
            "check_badge_standard.py docstring must keep " + leftover_272_doc + " pin",
            errors,
        )
    not_schema_282 = "NOT stewardship-schema leftover residual " + "#282"
...
def check_wiki_outline_gate_contract(errors: list[str]) -> None:
    ...
    leftover_272_wiki = "Wiki/mdlink leftover residual after " + "#272"
    if leftover_272_wiki not in wiki_text:
        fail(
            "check_wiki_outline.py docstring must keep " + leftover_272_wiki + " pin",
            errors,
        )
    not_schema_282_w = "NOT stewardship-schema leftover residual " + "#282"
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    # Fail-closed after #272: markdown link residual harden (wiki/mdlink leftover
    # residual; lands closed #281/#277 on post-#278 tip; NOT lychee/blob-503 leftover #278;
    # NOT Pass-2 residual + templates #272 /
    # NOT path-edges leftover #262 / NOT wiki/mdlink leftover #252 /
    # NOT stewardship-schema leftover #258 /
    # NOT path-filter/path-order leftover #244 / NOT md/link residual layouts #239).
    leftover_272_rel = "Wiki/mdlink leftover residual after " + "#272"
    if leftover_272_rel not in text:
        fail(
            "check_relative_links.py docstring must keep " + leftover_272_rel + " pin",
            errors,
        )
    not_schema_282_rel = "NOT stewardship-schema leftover residual " + "#282"
    if not_schema_282_rel not in text:
        fail(
            "check_relative_links.py docstring must keep " + not_schema_282_rel + " pin",
            errors,
        )
    not_lychee_278_rel = "NOT lychee/blob-503 leftover " + "#278"
    if not_lychee_278_rel not in text:
        fail(
            "check_relative_links.py docstring must keep " + not_lychee_278_rel + " pin",
            errors,
        )
