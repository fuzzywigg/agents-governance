...
def check_wiki_outline_gate_contract(errors: list[str]) -> None:
    ...
    # Fail-closed after #179: wiki-index validators (Home TOC empty-index /
    # publishable page index stubs) — distinct from wiki-badge after #141 /
    # fixtures #173 / path-filter #176 / run_stewardship #179 spam.
    # Split literals so self-mutation of contiguous names cannot neutralize checks.
    wiki_index_doc = "Wiki-index after " + "#179"
    if wiki_index_doc not in wiki_text:
        fail(
            "check_wiki_outline.py docstring must keep Wiki-index after #179 pin",
            errors,
        )
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    # Fail-closed after #179: wiki-index validators — broken internal stub
    # links / empty markdown index / duplicate slug headings_in set collapse
    # (not wiki-badge #141 / fixtures #173 / path-filter #176 / run_stewardship #179 spam).
    # Split literals so self-mutation of contiguous names cannot neutralize checks.
    rel_wiki_index = "Wiki-index after " + "#179"
    if rel_wiki_index not in text:
        fail(
            "check_relative_links.py docstring must keep Wiki-index after #179 pin",
            errors,
        )
