...
Fail-closed Pass-2 residual / templates leftover after #299 tip
...
lands closed #305/#298/#291 leftover on post-#299 tip — do not revive #305/#298/#291):
...
def check_lycheeignore(errors: list[str]) -> None:
    ...
    # Pass-2 residual / templates leftover after #299 tip (existing lychee
...
def check_agents_repo_template(errors: list[str]) -> None:
    ...
    # Pass-2 residual / templates leftover after #299 tip (existing
    ...
    for needle, label in leftover_299_required:
        if needle not in text:
            fail(
                f"templates/AGENTS-REPO.md must keep {label} "
                "(Pass-2 residual / templates leftover after #299 tip)",
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    if not RUN_STEWARDSHIP.is_file():
        ...
        # Pass-2 residual / templates leftover after #299 tip (soft-fail
...
def check_run_stewardship_gate_contract(errors: list[str]) -> None:
    ...
    if leftover_299 not in text:
        fail(
            "run_stewardship pins must keep " + leftover_299 + " marker",
            errors,
        )
