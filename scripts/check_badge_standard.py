...
def check_stewardship_common_contract(errors: list[str]) -> None:
    """Fail-close live stewardship_common wiring (after #111; deepen after #65/#46; leftover after #252; leftover residual after #262)."""
...
# Fail-closed after #293: schema residual CI helper / constant / needle pins
    ...
    # not schema leftover residual #282 /
    ...
    not_wiki_residual_293 = "NOT wiki/mdlink leftover residual " + "#293"
    if not_wiki_residual_293 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_wiki_residual_293,
            errors,
        )
    not_schema_258_rci = "NOT schema leftover " + "#258"
    if not_schema_258_rci not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_schema_258_rci,
            errors,
        )
    not_schema_residual_282_rci = "NOT schema leftover residual " + "#282"
    if not_schema_residual_282_rci not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_schema_residual_282_rci,
            errors,
        )
    not_lychee_278_rci = "NOT lychee/blob-503 leftover " + "#278"
    if not_lychee_278_rci not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_lychee_278_rci,
            errors,
        )
    not_pass2_272_rci = "NOT Pass-2 residual leftover " + "#272"
    if not_pass2_272_rci not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_pass2_272_rci,
            errors,
        )
    not_path_leftover_262_rci = "NOT path-filter/path-order leftover " + "#262"
    if not_path_leftover_262_rci not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_path_leftover_262_rci,
            errors,
        )
    not_wiki_252_rci = "NOT wiki/mdlink leftover " + "#252"
    if not_wiki_252_rci not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_wiki_252_rci,
            errors,
        )
    not_path_244_rci = "NOT path-edges residual " + "#244"
    if not_path_244_rci not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_path_244_rci,
            errors,
        )
    not_pass5_233_rci = "NOT schema residual (pass-5) " + "#233"
    if not_pass5_233_rci not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_pass5_233_rci,
            errors,
        )
    if_errors = "if errors" + ":"
    ...
    self_text_rci = BADGE_GATE.read_text(encoding="utf-8")
    if residual_ci_host not in self_text_rci:
        fail(
            "check_badge_standard.py must keep " + residual_ci_host + " host pin",
            errors,
        )
    residual_ci_slice = "schema residual CI " + "slice"
    if residual_ci_slice not in self_text_rci:
        fail(
    ...
    if contract_residual_ci not in self_text_rci:
        fail(
            "check_stewardship_schema_gate_contract docstring must pin "
