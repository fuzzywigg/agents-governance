...
Fail-closed Pass-2 residual / templates leftover after #297 tip
...
lands closed #298/#291 leftover on post-#297 tip — do not revive #298/#291):
...
def check_lycheeignore(errors: list[str]) -> None:
    ...
    # Pass-2 residual / templates leftover after #297 tip (existing lychee
    # module only; NOT saturated lychee/blob-503 leftover #278 /
    # NOT saturated Pass-2 residual leftover #272 /
    # NOT wiki/mdlink leftover residual #293 /
    # NOT stewardship-schema residual CI #297 / do not invent templates).
...
def check_agents_repo_template(errors: list[str]) -> None:
    ...
    # Pass-2 residual / templates leftover after #297 tip (existing
    # templates/AGENTS-REPO.md only; NOT saturated leftover #272 /
    # NOT lychee/blob-503 #278 / NOT wiki/mdlink leftover residual #293 /
    # NOT stewardship-schema residual CI #297; do not invent).
    leftover_297_required = (
    ...
    for needle, label in leftover_297_required:
        if needle not in text:
            fail(
                f"templates/AGENTS-REPO.md must keep {label} "
                "(Pass-2 residual / templates leftover after #297 tip)",
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    if not RUN_STEWARDSHIP.is_file():
        ...
        # Pass-2 residual / templates leftover after #297 tip (soft-fail
...
def check_run_stewardship_gate_contract(errors: list[str]) -> None:
    ...
    # Pass-2 residual / templates leftover after #297 tip contract pins.
    leftover_297 = (
        "Pass-2 residual / templates leftover after " + "#297 tip"
    )
    if leftover_297 not in text:
        fail(
            "run_stewardship pins must keep " + leftover_297 + " marker",
            errors,
        )
    not_schema_297 = "NOT stewardship-schema residual CI " + "#297"
    if not_schema_297 not in text:
        fail(
            "run_stewardship docstring must keep " + not_schema_297 + " distinctness",
            errors,
        )
    not_wiki_293 = "NOT wiki/mdlink leftover residual " + "#293"
    if not_wiki_293 not in text:
