...
Fail-closed wiki/mdlink leftover residual after #258 (beyond #252 saturated
after_243 pins; NOT wiki/mdlink leftover #252 / NOT stewardship-schema leftover #258 /
...
# Fail-closed after #227: markdown-lint/link-check residual exact layouts
    ...
    # Wiki/mdlink leftover residual after #258: md/link residual harden beyond #252.
    link_uses_adj = (
        "      - name: Check links\n"
    ...
    if link_uses_adj not in link:
        fail(
            ...
            "(wiki/mdlink leftover residual after #258)",
            errors,
        )
    lint_uses_adj = (
        "      - name: Run markdownlint\n"
...
def check_badge_standard_gate_contract(errors: list[str]) -> None:
    ...
    leftover_258_doc = "wiki/mdlink leftover residual after " + "#258"
    if leftover_258_doc not in text:
        fail(
            "check_badge_standard.py docstring must keep " + leftover_258_doc + " pin",
            errors,
        )
...
def check_wiki_outline_gate_contract(errors: list[str]) -> None:
    ...
    leftover_258_wiki = "Wiki/mdlink leftover residual after " + "#258"
    if leftover_258_wiki not in wiki_text:
        fail(
            "check_wiki_outline.py docstring must keep " + leftover_258_wiki + " pin",
            errors,
        )
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    # Fail-closed after #258: markdown link residual harden (wiki/mdlink leftover
    # residual; NOT wiki/mdlink leftover #252 / NOT stewardship-schema leftover #258 /
    # NOT path-filter/path-order leftover #244 / NOT md/link residual layouts #239).
    # Split literals so self-mutation of contiguous names cannot neutralize checks.
    leftover_258_rel = "Wiki/mdlink leftover residual after " + "#258"
    if leftover_258_rel not in text:
        fail(
            "check_relative_links.py docstring must keep " + leftover_258_rel + " pin",
            errors,
        )
