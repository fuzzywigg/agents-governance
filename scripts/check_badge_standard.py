...
Fail-closed actionlint path-order residual leftover deepen after #337
(NOT Pass-2 residual templates leftovers #337 / NOT wiki/mdlink leftover residual #326 / NOT schema residual CI leftover residual #320 / NOT saturated leftover #314 / NOT schema residual CI leftover #309 /
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    contiguous six-step actionlint path-order.
    Path-order residual leftover deepen after #337 (NOT Pass-2 residual templates leftovers #337 / NOT wiki/mdlink leftover residual #326 /
    ...
    # Path-order residual leftover deepen after #337 (DISTINCT leftover;
    # NOT Pass-2 residual templates leftovers #337 / NOT wiki/mdlink leftover residual #326 /
    # NOT schema residual CI leftover residual #320 /
    ...
    # NOT md/link residual #239 / NOT stewardship-checks/schema leftover #233 /
    # NOT wiki-badge leftover #227 / NOT stewardship-badge lint #208).
    checkout_idx_337 = stew.find("uses: actions/checkout@v7")
    setup_idx_337 = stew.find("name: Set up Python")
    install_idx_337 = stew.find("name: Install PyYAML (schema parser)")
    gates_name_337 = (
        "name: Stewardship gates (badge / wiki / schema / relative links)"
    )
    gates_idx_337 = stew.find(gates_name_337)
    self_tests_idx_337 = stew.find("name: Stewardship gate self-tests")
    dl_idx_337 = stew.find("name: Download actionlint")
    run_idx_337 = stew.find("name: actionlint existing workflow paths")
    if (
        checkout_idx_337 < 0
        or setup_idx_337 < 0
        or install_idx_337 < 0
        or gates_idx_337 < 0
        or self_tests_idx_337 < 0
        or dl_idx_337 < 0
        or run_idx_337 < 0
        or not (
            checkout_idx_337
    ...
    if setup_install_337 not in stew:
        fail(
    ...
    if install_gates_337 not in stew:
        fail(
...
def check_workflow_hardening_gate_contract(errors: list[str]) -> None:
    ...
    # Path-order residual leftover deepen after #337 (DISTINCT leftover).
    path_leftover_337_doc = (
        "Path-order residual leftover deepen after " + "#337"
    )
    if path_leftover_337_doc not in text:
        fail(
            "check_workflow_hardening docstring must pin " + path_leftover_337_doc,
            errors,
        )
    path_module_leftover_337 = (
        "path-order residual leftover deepen after " + "#337"
    )
    if path_module_leftover_337 not in text:
        fail(
            "check_badge_standard.py module docstring must pin "
            + path_module_leftover_337,
            errors,
    ...
    if not_pass2_templates_337 not in text:
        fail(
            "path leftover-after-337 must keep "
            + not_pass2_templates_337
            + " distinctness pin",
            errors,
        )
    ...
    if not_wiki_mdlink_337 not in text:
        fail(
            "path leftover-after-337 must keep "
            + not_wiki_mdlink_337
            + " distinctness pin",
    ...
    if not_schema_320_337 not in text:
        fail(
            "path leftover-after-337 must keep "
            + not_schema_320_337
            + " distinctness pin",
    ...
    for needle, label in path_leftover_337_pins:
        if needle not in text:
            fail(
                "check_workflow_hardening must keep " + label,
                errors,
