...
Fail-closed actionlint path-filter/path-order residual leftover deepen after #293
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    Path-filter/path-order residual leftover deepen after #293 (NOT wiki/mdlink
    ...
    # Path-filter/path-order residual leftover deepen after #293 (DISTINCT leftover;
    # NOT wiki/mdlink leftover residual #293 / NOT stewardship-schema residual #282 /
    # NOT lychee/blob-503 harden #278 / NOT Pass-2 residual leftover #272 /
    # NOT saturated leftover #262 /
    # NOT stewardship-schema leftover #258 /
    ...
    for wf_name, body in (
        ...
        if dispatch_concurrency not in body:
            fail(
                ...
                "adjacency "
                "(path-filter/path-order residual leftover deepen after #293)",
                errors,
            )
    ...
    setup_name_293 = "name: Set up Python"
    setup_idx_293 = stew.find(setup_name_293)
    install_idx_293 = stew.find("name: Install PyYAML (schema parser)")
    gates_name_293 = (
        "name: Stewardship gates (badge / wiki / schema / relative links)"
    )
    gates_idx_293 = stew.find(gates_name_293)
    self_tests_idx_293 = stew.find("name: Stewardship gate self-tests")
    dl_idx_293 = stew.find("name: Download actionlint")
    run_idx_293 = stew.find("name: actionlint existing workflow paths")
    if (
        setup_idx_293 < 0
        or install_idx_293 < 0
        or gates_idx_293 < 0
        or self_tests_idx_293 < 0
        or dl_idx_293 < 0
        or run_idx_293 < 0
        or not (
            setup_idx_293
...
def check_workflow_hardening_gate_contract(errors: list[str]) -> None:
    ...
    # Path-filter/path-order residual leftover deepen after #293 (DISTINCT leftover).
    path_leftover_293_doc = (
        "Path-filter/path-order residual leftover deepen after " + "#293"
    )
    if path_leftover_293_doc not in text:
        fail(
            "check_workflow_hardening docstring must pin " + path_leftover_293_doc,
            errors,
        )
    path_module_leftover_293 = (
        "path-filter/path-order residual leftover deepen after " + "#293"
    )
    if path_module_leftover_293 not in text:
        fail(
            "check_badge_standard.py module docstring must pin "
            + path_module_leftover_293,
            errors,
    ...
    if not_wiki_mdlink_293 not in text:
        fail(
            "path leftover-after-293 must keep "
            + not_wiki_mdlink_293
            + " distinctness pin",
            errors,
        )
    ...
    not_schema_leftover_258_293 = "NOT stewardship-schema leftover " + "#258"
    if not_schema_leftover_258_293 not in text:
        fail(
            "path leftover-after-293 must keep "
            + not_schema_leftover_258_293
            + " distinctness pin",
            errors,
        )
    not_wiki_mdlink_252_293 = "NOT wiki/mdlink leftover " + "#252"
    if not_wiki_mdlink_252_293 not in text:
        fail(
            "path leftover-after-293 must keep "
            + not_wiki_mdlink_252_293
            + " distinctness pin",
            errors,
        )
    not_saturated_residual_244_293 = "NOT saturated residual " + "#244"
    if not_saturated_residual_244_293 not in text:
        fail(
            "path leftover-after-293 must keep "
            + not_saturated_residual_244_293
            + " distinctness pin",
            errors,
        )
    not_wiki_publish_243_293 = "NOT wiki outline/PUBLISH leftover " + "#243"
    if not_wiki_publish_243_293 not in text:
        fail(
            "path leftover-after-293 must keep "
            + not_wiki_publish_243_293
            + " distinctness pin",
            errors,
        )
    not_mdlink_239_293 = "NOT md/link residual " + "#239"
    if not_mdlink_239_293 not in text:
        fail(
            "path leftover-after-293 must keep "
            + not_mdlink_239_293
            + " distinctness pin",
    ...
    not_stew_schema_233_293 = (
        "NOT stewardship-checks/schema leftover " + "#233"
    )
    if not_stew_schema_233_293 not in text:
        fail(
            "path leftover-after-293 must keep "
            + not_stew_schema_233_293
            + " distinctness pin",
    ...
    not_wiki_badge_227_293 = "NOT wiki-badge leftover " + "#227"
    if not_wiki_badge_227_293 not in text:
        fail(
            "path leftover-after-293 must keep "
            + not_wiki_badge_227_293
            + " distinctness pin",
            errors,
        )
    not_saturated_deepen_225_293 = "NOT saturated deepen " + "#225"
    if not_saturated_deepen_225_293 not in text:
        fail(
            "path leftover-after-293 must keep "
            + not_saturated_deepen_225_293
            + " distinctness pin",
            errors,
        )
    not_pass2_220_leftover_293 = "NOT Pass-2 leftover + " + "md/link #220"
    if not_pass2_220_leftover_293 not in text:
        fail(
            "path leftover-after-293 must keep "
            + not_pass2_220_leftover_293
            + " distinctness pin",
            errors,
        )
    path_leftover_293_pins = (
        (
            "contiguous workflow_dispatch:/concurrency: " + "adjacency",
        ...
        (
            "path-filter/path-order residual leftover deepen after " + "#293",
            "path leftover-after-293 wording pin",
        ),
    )
    for needle, label in path_leftover_293_pins:
        if needle not in text:
            fail(
                "check_workflow_hardening must keep " + label,
                errors,
...
def check_stewardship_schema_gate_contract(errors: list[str]) -> None:
    """Fail-close live stewardship-schema gate wiring (after #72/#75; third-pass after #132; deepen after #189; policy-schema pass-4 after #208; schema residual after #225; leftover after #252; leftover residual after #262)."""
