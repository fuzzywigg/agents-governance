...
- Path-order leftover after #176 (lands closed #157; NOT #165 stewardship CI /
  NOT path-filter / NOT badge-lint / NOT #149 cancel-in-progress):
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    Path-order leftover after #176 (lands closed #157; NOT #165 stewardship CI /
    NOT path-filter / NOT badge-lint): contiguous three-path actionlint order /
    exact bash <(curl -fsSL) download / no continue-on-error: true /
...
def check_workflow_hardening_gate_contract(errors: list[str]) -> None:
    ...
    # Path-order leftover after #176 (lands closed #157; NOT #165 stewardship CI /
    # NOT path-filter / NOT badge-lint).
    path_order_doc = "Path-order leftover after " + "#176"
    if path_order_doc not in text:
        fail(
            "check_workflow_hardening docstring must pin " + path_order_doc,
            errors,
        )
    module_path_order = "Path-order leftover after " + "#176"
