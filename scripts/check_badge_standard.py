...
Fail-closed wiki/mdlink leftover residual after #299 tip (beyond #293 saturated
after_293 pins; lands #307/#302 leftover; NOT stewardship-schema residual CI #299 /
...
# Fail-closed after #227: markdown-lint/link-check residual exact layouts
    ...
    # Wiki/mdlink leftover residual after #299 tip: md/link residual harden beyond #293 (lands #307/#302).
    link_token_args_adj = (
        "          token: ${{ secrets.GITHUB_TOKEN }}\n"
        "          args: >-"
    )
    if link_token_args_adj not in link:
        fail(
            ...
            "(wiki/mdlink leftover residual after #299)",
            errors,
        )
    link_steps_triple = (
        "      - uses: actions/checkout@v7\n"
    ...
    if lint_steps_triple not in lint:
        fail(
            ...
            "triple adjacency (wiki/mdlink leftover residual after #299)",
            errors,
        )
...
def check_badge_standard_gate_contract(errors: list[str]) -> None:
    ...
    leftover_299_doc = "wiki/mdlink leftover residual after " + "#299"
    if leftover_299_doc not in text:
        fail(
            "check_badge_standard.py docstring must keep " + leftover_299_doc + " pin",
            errors,
        )
...
def check_wiki_outline_gate_contract(errors: list[str]) -> None:
    ...
    leftover_299_wiki = "Wiki/mdlink leftover residual after " + "#299"
    if leftover_299_wiki not in wiki_text:
        fail(
            "check_wiki_outline.py docstring must keep " + leftover_299_wiki + " pin",
            errors,
        )
