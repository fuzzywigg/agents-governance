...
Fail-closed actionlint path-filter/path-order residual leftover deepen after #309
(NOT schema residual CI leftover #309 / NOT schema residual CI #299 / NOT wiki/mdlink leftover residual #293 /
...
- schema leftover residual after #262: leftover residual helper needles + leftover residual status/surface stubs (schema leftover residual slice)
  (NOT path-filter/path-order leftover #262 / NOT schema leftover #258 / NOT wiki/mdlink leftover #252 / NOT path-edges residual #244 / NOT wiki outline/PUBLISH #243 / NOT md/link residual #239 / NOT schema residual (pass-5) #233)
- common leftover after #252: elif hint in text / for pattern in globs /
  found: set[Path] = set() / if not path.is_file(): (load_workflow)
- common leftover residual after #262: errors.append(msg) / lowered = target.strip().lower() /
  for scheme in DANGEROUS_LINK_SCHEMES: / found.update(ROOT.glob(pattern))
- schema residual CI deepen after #293: residual CI helper needles + residual CI status/surface stubs (schema residual CI slice)
...
- schema residual deepen after #326: residual helper needles + residual status/surface stubs (schema residual slice)
  (NOT Pass-2 residual templates leftovers #337 / NOT Pass-2 residual / templates leftovers after #337 #348 / NOT wiki/mdlink leftover residual #326 / NOT schema residual CI leftover residual #320 / NOT schema residual CI leftover #309 / NOT schema residual CI #299 / NOT wiki/mdlink leftover residual #293 / NOT schema leftover residual #282 / NOT lychee/blob-503 leftover #278 / NOT Pass-2 residual leftover #272 / NOT path-filter/path-order leftover #262 / NOT schema leftover #258 / NOT wiki/mdlink leftover #252 / NOT path-edges residual #244 / NOT schema residual (pass-5) #233)
...
Fail-closed Pass-2 residual / templates leftover after #337 tip
(beyond #326 saturated after_326 pins; NOT Pass-2 residual / templates leftover after #326 /
NOT wiki/mdlink leftover residual #326 /
NOT stewardship-schema residual CI leftover residual #320 /
NOT path-filter/path-order residual leftover #314 /
NOT stewardship-schema residual CI leftover #309 /
...
NOT path-edges leftover #262 / NOT schema leftover #258 /
NOT wiki/mdlink leftover #252; do not invent new templates;
leftover residual on post-#337 tip — do not revive #333/#331/#324/#315/#313/#312/#305/#298/#291):
- soft-fail with || /bin/true / soft-fail with || /usr/bin/true /
  soft-fail with set +E
- existing templates/AGENTS-REPO.md leftover residual:
  [Section Title] / [Add project-specific governance rules here] /
  ### Environments / Environment|Platform|Status header /
  Development Local row / Staging [Platform] [Status] /
  Production [Platform] [Status] /
  Human confirms every action /
  Auto-approve within policy + budget /
  Human approval required /
  Immediate response / < 1 hour / < 24 hours / Next sprint /
  ### Version History / For ecosystem-wide governance /
  See [agents-governance] footer
- lychee residual: reject invent blob/master exclude /
  reject invent tree/master exclude /
...
Fail-closed wiki/mdlink leftover residual after #320 tip (beyond #293 saturated
after_293 pins; lands #325/#318/#308/#307/#302 leftover; NOT stewardship-schema residual CI leftover residual #320 /
...
def check_lycheeignore(errors: list[str]) -> None:
    ...
    # Pass-2 residual / templates leftover after #337 tip (existing lychee
    # module only; beyond #326 saturated tree/main + bare github.com;
    ...
    if "blob/master" in text:
        fail(
            ".lycheeignore must not invent blob/master exclude "
            "(Pass-2 residual / templates leftover after #337 tip; "
            "blob/main HTML flake only)",
            errors,
        )
    if "tree/master" in text:
        fail(
            ".lycheeignore must not invent tree/master exclude "
            "(Pass-2 residual / templates leftover after #337 tip; "
            "blob/main HTML flake only)",
            errors,
        )
    if "raw.githubusercontent.com" in text:
        fail(
            ".lycheeignore must not invent raw.githubusercontent.com exclude "
            "(Pass-2 residual / templates leftover after #337 tip; "
            "blob/main HTML flake only)",
            errors,
        )
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    Path-filter/path-order residual leftover deepen after #309 (NOT schema residual CI leftover
    #309 / NOT schema residual CI #299 / NOT wiki/mdlink leftover residual #293 / NOT stewardship-schema residual #282 /
    NOT lychee/blob-503 harden #278 / NOT Pass-2 residual leftover #272 /
    NOT saturated leftover #262 /
    NOT stewardship-schema leftover #258 /
    NOT wiki/mdlink leftover #252 / NOT saturated residual #244 /
    ...
    # Path-filter/path-order residual leftover deepen after #309 (DISTINCT leftover;
    # NOT schema residual CI leftover #309 / NOT schema residual CI #299 / NOT wiki/mdlink leftover residual #293 /
    # NOT stewardship-schema residual #282 /
    # NOT lychee/blob-503 harden #278 / NOT Pass-2 residual leftover #272 /
    # NOT saturated leftover #262 /
    ...
    setup_name_309 = "name: Set up Python"
    setup_idx_309 = stew.find(setup_name_309)
    install_idx_309 = stew.find("name: Install PyYAML (schema parser)")
    gates_name_309 = (
        "name: Stewardship gates (badge / wiki / schema / relative links)"
    )
    gates_idx_309 = stew.find(gates_name_309)
    self_tests_idx_309 = stew.find("name: Stewardship gate self-tests")
    dl_idx_309 = stew.find("name: Download actionlint")
    run_idx_309 = stew.find("name: actionlint existing workflow paths")
    if (
        setup_idx_309 < 0
        or install_idx_309 < 0
        or gates_idx_309 < 0
        or self_tests_idx_309 < 0
        or dl_idx_309 < 0
        or run_idx_309 < 0
        or not (
            setup_idx_309
...
# Fail-closed after #227: markdown-lint/link-check residual exact layouts
    ...
    # Wiki/mdlink leftover residual after #320 tip: md/link residual harden beyond #293 (lands #325/#318/#308/#307/#302; NOT schema residual CI leftover residual #320).
    link_token_args_adj = (
        "          token: ${{ secrets.GITHUB_TOKEN }}\n"
        "          args: >-"
    )
    if link_token_args_adj not in link:
        fail(
            ...
            "(wiki/mdlink leftover residual after #320)",
            errors,
        )
    link_steps_triple = (
        "      - uses: actions/checkout@v7\n"
    ...
    if lint_steps_triple not in lint:
        fail(
            ...
            "triple adjacency (wiki/mdlink leftover residual after #320)",
            errors,
        )
...
def check_agents_repo_template(errors: list[str]) -> None:
    ...
    # Pass-2 residual / templates leftover after #337 tip (existing
    # templates/AGENTS-REPO.md only; beyond #326 saturated autonomy/SEV pins;
    # NOT Pass-2 residual / templates leftover after #326 /
    # NOT saturated leftover #272 / NOT lychee/blob-503 #278 /
    # NOT wiki/mdlink leftover residual #326 /
    # NOT path-filter/path-order residual leftover #314 /
    # NOT stewardship-schema residual CI leftover residual #320; do not invent).
    leftover_337_required = (
        ("[Section Title]", "[Section Title] placeholder"),
        (
            "[Add project-specific governance rules here]",
            "[Add project-specific governance rules here] placeholder",
        ),
        ("### Environments", "Environments"),
        (
            "| Environment | Platform | Status |",
            "Environment Platform Status header",
        ),
        ("| Development | Local | ✅ |", "Development Local row"),
        (
            "| Staging | [Platform] | [Status] |",
            "Staging [Platform] [Status] row",
        ),
        (
            "| Production | [Platform] | [Status] |",
            "Production [Platform] [Status] row",
        ),
        ("Human confirms every action", "Human confirms every action"),
        (
            "Auto-approve within policy + budget",
            "Auto-approve within policy + budget",
        ),
        ...
        (
            "See [agents-governance]",
            "See [agents-governance] footer",
        ),
    )
    for needle, label in leftover_337_required:
        if needle not in text:
            fail(
                f"templates/AGENTS-REPO.md must keep {label} "
                "(Pass-2 residual / templates leftover after #337 tip)",
...
def check_badge_standard_gate_contract(errors: list[str]) -> None:
    ...
    leftover_320_doc = "wiki/mdlink leftover residual after " + "#320"
    if leftover_320_doc not in text:
        fail(
            "check_badge_standard.py docstring must keep " + leftover_320_doc + " pin",
            errors,
        )
...
def check_workflow_hardening_gate_contract(errors: list[str]) -> None:
    ...
    # Path-filter/path-order residual leftover deepen after #309 (DISTINCT leftover).
    path_leftover_309_doc = (
        "Path-filter/path-order residual leftover deepen after " + "#309"
    )
    if path_leftover_309_doc not in text:
        fail(
            "check_workflow_hardening docstring must pin " + path_leftover_309_doc,
            errors,
        )
    path_module_leftover_309 = (
        "path-filter/path-order residual leftover deepen after " + "#309"
    )
    if path_module_leftover_309 not in text:
        fail(
            "check_badge_standard.py module docstring must pin "
            + path_module_leftover_309,
            errors,
    ...
    if not_schema_residual_ci_leftover_309 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_schema_residual_ci_leftover_309
            + " distinctness pin",
            errors,
        )
    not_schema_residual_ci_299 = "NOT schema residual CI " + "#299"
    if not_schema_residual_ci_299 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_schema_residual_ci_299
            + " distinctness pin",
            errors,
        )
    not_wiki_mdlink_293 = "NOT wiki/mdlink leftover residual " + "#293"
    ...
    not_schema_residual_282 = "NOT stewardship-schema residual " + "#282"
    if not_schema_residual_282 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_schema_residual_282
            + " distinctness pin",
            errors,
        )
    ...
    not_saturated_leftover_262 = "NOT saturated leftover " + "#262"
    if not_saturated_leftover_262 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_saturated_leftover_262
            + " distinctness pin",
            errors,
        )
    not_schema_leftover_258_297 = "NOT stewardship-schema leftover " + "#258"
    if not_schema_leftover_258_297 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_schema_leftover_258_297
            + " distinctness pin",
            errors,
        )
    not_wiki_mdlink_252_297 = "NOT wiki/mdlink leftover " + "#252"
    if not_wiki_mdlink_252_297 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_wiki_mdlink_252_297
            + " distinctness pin",
            errors,
        )
    not_saturated_residual_244_297 = "NOT saturated residual " + "#244"
    if not_saturated_residual_244_297 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_saturated_residual_244_297
            + " distinctness pin",
            errors,
        )
    not_wiki_publish_243_297 = "NOT wiki outline/PUBLISH leftover " + "#243"
    if not_wiki_publish_243_297 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_wiki_publish_243_297
            + " distinctness pin",
            errors,
        )
    not_mdlink_239_297 = "NOT md/link residual " + "#239"
    if not_mdlink_239_297 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_mdlink_239_297
            + " distinctness pin",
    ...
    not_stew_schema_233_297 = (
        "NOT stewardship-checks/schema leftover " + "#233"
    )
    if not_stew_schema_233_297 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_stew_schema_233_297
            + " distinctness pin",
    ...
    not_wiki_badge_227_297 = "NOT wiki-badge leftover " + "#227"
    if not_wiki_badge_227_297 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_wiki_badge_227_297
            + " distinctness pin",
            errors,
        )
    not_saturated_deepen_225_297 = "NOT saturated deepen " + "#225"
    if not_saturated_deepen_225_297 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_saturated_deepen_225_297
            + " distinctness pin",
            errors,
        )
    not_pass2_220_leftover_297 = "NOT Pass-2 leftover + " + "md/link #220"
    if not_pass2_220_leftover_297 not in text:
        fail(
            "path leftover-after-309 must keep "
            + not_pass2_220_leftover_297
            + " distinctness pin",
            errors,
        )
    path_leftover_309_pins = (
        (
            "contiguous workflow_dispatch:/concurrency: " + "adjacency",
        ...
        (
            "path-filter/path-order residual leftover deepen after " + "#309",
            "path leftover-after-309 wording pin",
        ),
    )
...
def check_stewardship_common_contract(errors: list[str]) -> None:
    """Fail-close live stewardship_common wiring (after #111; deepen after #65/#46; leftover after #252; leftover residual after #262; residual CI deepen after #293; residual CI leftover deepen after #299; residual CI leftover residual deepen after #309; residual deepen after #326 tip-relaunch on post-#348)."""
    ...
    # Fail-closed after #262: common leftover residual helper / constant / needle pins
    # (common leftover residual slice only; not path-filter/path-order leftover #262 /
    # not schema leftover #258 / not wiki/mdlink leftover #252 /
    # not path-edges residual #244 / not wiki outline/PUBLISH #243 / not md/link residual #239 /
    # not schema residual (pass-5) #233 spam).
    # Split literals so self-mutation of contiguous names cannot neutralize checks.
    common_leftover_262_doc = "Residual leftover after " + "#262 (common leftover residual"
    if common_leftover_262_doc not in text:
        fail(
            "stewardship_common.py docstring must pin " + common_leftover_262_doc,
            errors,
        )
    errors_append = "errors.append(" + "msg)"
    if errors_append not in text:
        fail(
            "stewardship_common.py fail must keep errors.append(msg)",
            errors,
        )
    lowered_target = "lowered = target.strip()." + "lower()"
    if lowered_target not in text:
        fail(
            "stewardship_common.py has_dangerous_scheme must keep lowered = target.strip().lower()",
            errors,
        )
    for_scheme = "for scheme in DANGEROUS_LINK_" + "SCHEMES:"
    if for_scheme not in text:
        fail(
            "stewardship_common.py has_dangerous_scheme must iterate for scheme in DANGEROUS_LINK_SCHEMES:",
            errors,
        )
    found_update = "found.update(ROOT.glob(" + "pattern))"
    if found_update not in text:
        fail(
            "stewardship_common.py markdown_files must keep found.update(ROOT.glob(pattern))",
            errors,
        )
    common_host_262 = "common leftover residual after " + "#262"
    self_text_common_262 = BADGE_GATE.read_text(encoding="utf-8")
    if common_host_262 not in self_text_common_262:
        fail(
            "check_badge_standard.py must keep " + common_host_262 + " host pin",
            errors,
        )
...
def check_stewardship_schema_gate_contract(errors: list[str]) -> None:
    """Fail-close live stewardship-schema gate wiring (after #72/#75; third-pass after #132; deepen after #189; policy-schema pass-4 after #208; schema residual after #225; leftover after #252; leftover residual after #262; residual CI deepen after #293; residual CI leftover deepen after #299; residual CI leftover residual deepen after #309; residual deepen after #326 tip-relaunch on post-#348)."""
    ...
    # Fail-closed after #262: schema leftover residual helper / constant / needle pins
    # (schema leftover residual slice only; not path-filter/path-order leftover #262 /
    # not schema leftover #258 / not wiki/mdlink leftover #252 /
    # not path-edges residual #244 / not wiki outline/PUBLISH #243 / not md/link residual #239 /
    # not schema residual (pass-5) #233 spam).
    # Split literals so self-mutation of contiguous names cannot neutralize checks.
    leftover_262_doc = "Residual leftover after " + "#262 (schema leftover residual"
    if leftover_262_doc not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + leftover_262_doc,
            errors,
        )
    not_path_leftover_262 = "NOT path-filter/path-order leftover " + "#262"
    if not_path_leftover_262 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_path_leftover_262,
            errors,
        )
    not_schema_leftover_258b = "NOT schema leftover " + "#258"
    if not_schema_leftover_258b not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_schema_leftover_258b,
            errors,
        )
    not_wiki_mdlink_252b = "NOT wiki/mdlink leftover " + "#252"
    if not_wiki_mdlink_252b not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_wiki_mdlink_252b,
            errors,
        )
    not_path_edges_244b = "NOT path-edges residual " + "#244"
    if not_path_edges_244b not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_path_edges_244b,
            errors,
        )
    not_wiki_243b = "NOT wiki outline/PUBLISH " + "#243"
    if not_wiki_243b not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_wiki_243b,
            errors,
        )
    not_mdlink_239b = "NOT md/link residual " + "#239"
    if not_mdlink_239b not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_mdlink_239b,
            errors,
        )
    not_pass5_233b = "NOT schema residual (pass-5) " + "#233"
    if not_pass5_233b not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_pass5_233b,
            errors,
        )
    ...
    data_assign = "data[key] = " + "value"
    if data_assign not in text:
        fail(
            "check_stewardship_schema.py parse_simple_yaml must keep data[key] = value",
            errors,
        )
    mapping_err = 'raise ValueError("metadata YAML must be a mapping")'
    if mapping_err not in text and "raise ValueError('metadata YAML must be a mapping')" not in text:
        fail(
            'check_stewardship_schema.py load_yaml must keep raise ValueError("metadata YAML must be a mapping")',
            errors,
        )
    match_group = "return match.group(" + "1)"
    if match_group not in text:
        fail(
            "check_stewardship_schema.py first_yaml_block must keep return match.group(1)",
            errors,
        )
    first_yaml = "block = first_yaml_block(" + "path)"
    if first_yaml not in text:
        fail(
            "check_stewardship_schema.py main must keep block = first_yaml_block(path)",
            errors,
        )
    load_yaml_call = "data = load_yaml(" + "block)"
    if load_yaml_call not in text:
        fail(
            "check_stewardship_schema.py main must keep data = load_yaml(block)",
            errors,
        )
    if_missing = "if " + "missing:"
    if if_missing not in text:
        fail(
            "check_stewardship_schema.py main must keep if missing:",
            errors,
        )
    got_get = "got = data.get(" + "key)"
    if got_get not in text:
        fail(
            "check_stewardship_schema.py EXPECTED_VALUES loop must keep got = data.get(key)",
            errors,
        )
    policy_lower = 'policy = str(data["edit_policy"]).lower()'
    if policy_lower not in text and "policy = str(data['edit_policy']).lower()" not in text:
        fail(
            'check_stewardship_schema.py edit_policy gate must keep policy = str(data["edit_policy"]).lower()',
            errors,
        )
    failed_banner = 'print("Stewardship schema check FAILED:"'
    if failed_banner not in text and "print('Stewardship schema check FAILED:'" not in text:
        fail(
            'check_stewardship_schema.py must keep print("Stewardship schema check FAILED:" banner',
            errors,
        )
    for stub in (
        "REJECTED",
        ...
        if stub not in text:
            fail(
                "check_stewardship_schema.py docstring must pin leftover residual status "
                f"enum stub {stub}",
                errors,
            )
    for stub in (
        ...
        if stub not in text:
            fail(
                "check_stewardship_schema.py docstring must pin leftover residual surface "
                f"enum stub {stub}",
                errors,
            )
    leftover_host_262 = "schema leftover residual after " + "#262"
    self_text6 = BADGE_GATE.read_text(encoding="utf-8")
    if leftover_host_262 not in self_text6:
        fail(
            "check_badge_standard.py must keep " + leftover_host_262 + " host pin",
            errors,
        )
    leftover_slice_262 = "schema leftover residual " + "slice"
    if leftover_slice_262 not in self_text6:
        fail(
            "check_badge_standard.py schema leftover residual must keep "
            + leftover_slice_262
            + " pin",
            errors,
        )
    ...
    # Fail-closed after #293: schema residual CI helper / constant / needle pins
    # (schema residual CI slice only; not wiki/mdlink leftover residual #293 /
    ...
    # Split literals so self-mutation of contiguous names cannot neutralize checks.
    residual_ci_293_doc = "Residual CI deepen after " + "#293 (schema residual CI"
    if residual_ci_293_doc not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + residual_ci_293_doc,
            errors,
        )
    not_wiki_293 = "NOT wiki/mdlink leftover residual " + "#293"
    if not_wiki_293 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_wiki_293,
            errors,
        )
    not_schema_258 = "NOT schema leftover " + "#258"
    if not_schema_258 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_schema_258,
            errors,
        )
    not_schema_residual_282 = "NOT schema leftover residual " + "#282"
    if not_schema_residual_282 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_schema_residual_282,
            errors,
        )
    not_lychee_278 = "NOT lychee/blob-503 leftover " + "#278"
    if not_lychee_278 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_lychee_278,
            errors,
        )
    not_pass2_272 = "NOT Pass-2 residual leftover " + "#272"
    if not_pass2_272 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_pass2_272,
            errors,
        )
    not_path_leftover_262 = "NOT path-filter/path-order leftover " + "#262"
    if not_path_leftover_262 not in text:
    ...
    not_path_244 = "NOT path-edges residual " + "#244"
    if not_path_244 not in text:
        fail(
    ...
    date_key_missing = "if date_key not in " + "data:"
    if date_key_missing not in text:
        fail(
            "check_stewardship_schema.py must keep if date_key not in data:",
            errors,
        )
    iso_match = "if not ISO_DATE_RE.match(" + "raw):"
    if iso_match not in text:
        fail(
            "check_stewardship_schema.py must keep if not ISO_DATE_RE.match(raw):",
            errors,
        )
    semver_match = "if not SEMVER_RE.match(" + "ver):"
    if semver_match not in text:
        fail(
            "check_stewardship_schema.py must keep if not SEMVER_RE.match(ver):",
            errors,
        )
    issue_search = "if not ISSUE_REF_RE.search(" + "closes):"
    if issue_search not in text:
        fail(
            "check_stewardship_schema.py must keep if not ISSUE_REF_RE.search(closes):",
            errors,
        )
    ...
    expected_items = "for key, want in expected.items(" + "):"
    if expected_items not in text:
        fail(
            "check_stewardship_schema.py must iterate for key, want in expected.items():",
            errors,
        )
    level_get = 'level = data["autonomy_level"]'
    if level_get not in text and "level = data['autonomy_level']" not in text:
        fail(
            'check_stewardship_schema.py must keep level = data["autonomy_level"]',
            errors,
        )
    err_print = 'print(f"  - {err}"'
    if err_print not in text:
        fail(
            'check_stewardship_schema.py FAILED path must print(f"  - {err}"',
            errors,
        )
    splitlines_pin = "for raw in text.splitlines(" + "):"
    if splitlines_pin not in text:
        fail(
            "check_stewardship_schema.py parse_simple_yaml must iterate for raw in text.splitlines():",
            errors,
        )
    line_strip = "line = raw.strip(" + ")"
    if line_strip not in text:
        fail(
            "check_stewardship_schema.py parse_simple_yaml must keep line = raw.strip()",
            errors,
        )
    key_strip = "key = key.strip(" + ")"
    if key_strip not in text:
        fail(
            "check_stewardship_schema.py parse_simple_yaml must keep key = key.strip()",
            errors,
        )
    nonempty_compound = "isinstance(value, str) and not value.strip(" + ")"
    if nonempty_compound not in text:
        fail(
            "check_stewardship_schema.py must keep isinstance(value, str) and not value.strip()",
            errors,
        )
    root_rel = "path = ROOT / " + "rel"
    if root_rel not in text:
        fail(
            "check_stewardship_schema.py must resolve docs via path = ROOT / rel",
            errors,
        )
    for stub in (
        "SHELVED",
        ...
        if stub not in text:
            fail(
                "check_stewardship_schema.py docstring must pin residual CI status "
                f"enum stub {stub}",
                errors,
            )
    for stub in (
        ...
        if stub not in text:
            fail(
                "check_stewardship_schema.py docstring must pin residual CI surface "
                f"enum stub {stub}",
                errors,
            )
    residual_ci_host = "schema residual CI deepen after " + "#293"
    self_text7 = BADGE_GATE.read_text(encoding="utf-8")
    if residual_ci_host not in self_text7:
        fail(
            "check_badge_standard.py must keep " + residual_ci_host + " host pin",
            errors,
        )
    residual_ci_slice = "schema residual CI " + "slice"
    if residual_ci_slice not in self_text7:
        fail(
            "check_badge_standard.py schema residual CI must keep "
            + residual_ci_slice
            + " pin",
            errors,
        )
    contract_residual_ci = "residual CI deepen after " + "#293"
    if contract_residual_ci not in self_text7:
        fail(
            "check_stewardship_schema_gate_contract docstring must pin "
            + contract_residual_ci,
            errors,
        )
    ...
    # Fail-closed after #299: schema residual CI leftover helper / constant / needle pins
    # (schema residual CI leftover slice only; not schema residual CI #299 /
    ...
    # not schema residual (pass-5) #233 spam).
    # Split literals so self-mutation of contiguous names cannot neutralize checks.
    residual_ci_leftover_299_doc = "Residual CI leftover deepen after " + "#299 (schema residual CI leftover"
    if residual_ci_leftover_299_doc not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + residual_ci_leftover_299_doc,
            errors,
        )
    not_schema_residual_ci_299 = "NOT schema residual CI " + "#299"
    if not_schema_residual_ci_299 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_schema_residual_ci_299,
            errors,
        )
    not_wiki_293_leftover = "NOT wiki/mdlink leftover residual " + "#293"
    if not_wiki_293_leftover not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_wiki_293_leftover,
            errors,
        )
    not_schema_leftover_residual_282 = "NOT schema leftover residual " + "#282"
    if not_schema_leftover_residual_282 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_schema_leftover_residual_282,
            errors,
        )
    not_pass2_272_leftover = "NOT Pass-2 residual leftover " + "#272"
    if not_pass2_272_leftover not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_pass2_272_leftover,
            errors,
        )
    not_path_leftover_262_b = "NOT path-filter/path-order leftover " + "#262"
    if not_path_leftover_262_b not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_path_leftover_262_b,
            errors,
        )
    missing_sorted = "missing = sorted(required_keys - set(data))"
    if missing_sorted not in text:
        fail(
            "check_stewardship_schema.py must keep missing = sorted(required_keys - set(data))",
            errors,
        )
    key_not_in = "if key not in " + "data:"
    if key_not_in not in text:
        fail(
            "check_stewardship_schema.py must keep if key not in data:",
            errors,
        )
    value_data_key = "value = data[" + "key]"
    if value_data_key not in text:
        fail(
            "check_stewardship_schema.py must keep value = data[key]",
            errors,
        )
    reject_non_scalar = "if reject_non_scalar(rel, key, value, errors)" + ":"
    if reject_non_scalar not in text:
        fail(
            "check_stewardship_schema.py must keep if reject_non_scalar(rel, key, value, errors):",
            errors,
        )
    status_get = 'status = data.get("status")'
    if status_get not in text and "status = data.get('status')" not in text:
        fail(
            'check_stewardship_schema.py must keep status = data.get("status")',
            errors,
        )
    status_active = 'str(status).upper() != "ACTIVE"'
    if status_active not in text and "str(status).upper() != 'ACTIVE'" not in text:
        fail(
            'check_stewardship_schema.py must keep str(status).upper() != "ACTIVE"',
            errors,
        )
    rel_startswith_docs = 'if rel.startswith("docs/")' + ":"
    if rel_startswith_docs not in text and "if rel.startswith('docs/'):" not in text:
        fail(
            'check_stewardship_schema.py must keep if rel.startswith("docs/"):',
            errors,
        )
    expected_get = "expected = EXPECTED_VALUES.get(rel, " + "{})"
    if expected_get not in text:
        fail(
            "check_stewardship_schema.py must keep expected = EXPECTED_VALUES.get(rel, {})",
            errors,
        )
    got_want = "if got != " + "want:"
    if got_want not in text:
        fail(
            "check_stewardship_schema.py must keep if got != want:",
            errors,
        )
    isinstance_level_bool = "isinstance(level, " + "bool)"
    if isinstance_level_bool not in text:
        fail(
            "check_stewardship_schema.py must keep isinstance(level, bool)",
            errors,
        )
    level_not_in = "level not in (0, 1, 2, " + "3)"
    if level_not_in not in text:
        fail(
            "check_stewardship_schema.py must keep level not in (0, 1, 2, 3)",
            errors,
        )
    tier_in_data = 'if "tier" in ' + "data:"
    if tier_in_data not in text and "if 'tier' in data:" not in text:
        fail(
            'check_stewardship_schema.py must keep if "tier" in data:',
            errors,
        )
    tier_get = 'tier = data["tier"]'
    if tier_get not in text and "tier = data['tier']" not in text:
        fail(
            'check_stewardship_schema.py must keep tier = data["tier"]',
            errors,
        )
    for_date_key = "for date_key in DATE_KEYS" + ":"
    if for_date_key not in text:
        fail(
            "check_stewardship_schema.py must keep for date_key in DATE_KEYS:",
            errors,
        )
    invent_not_in = 'if "invent" not in ' + "policy"
    if invent_not_in not in text and "if 'invent' not in policy" not in text:
        fail(
            'check_stewardship_schema.py must keep if "invent" not in policy',
            errors,
        )
    scan_secrets_pin = "scan_secrets(path, " + "errors)"
    if scan_secrets_pin not in text:
        fail(
            "check_stewardship_schema.py must keep scan_secrets(path, errors)",
            errors,
        )
    except_exc = "except Exception as " + "exc:"
    if except_exc not in text:
        fail(
            "check_stewardship_schema.py must keep except Exception as exc:",
            errors,
        )
    errors_list = "errors: list[str] = " + "[]"
    if errors_list not in text:
        fail(
            "check_stewardship_schema.py must keep errors: list[str] = []",
            errors,
        )
    sys_exit_main = "sys.exit(main(" + "))"
    if sys_exit_main not in text:
        fail(
            "check_stewardship_schema.py must keep sys.exit(main())",
            errors,
        )
    engine_pyyaml = 'engine = "PyYAML" if yaml is not None else "stdlib-subset"'
    if engine_pyyaml not in text:
        fail(
            "check_stewardship_schema.py must keep engine = PyYAML-or-stdlib-subset pin",
            errors,
        )
    for stub in (
        "BLOCKED",
        ...
        if stub not in text:
            fail(
                "check_stewardship_schema.py docstring must pin residual CI leftover status "
                f"enum stub {stub}",
                errors,
            )
    for stub in (
        ...
        if stub not in text:
            fail(
                "check_stewardship_schema.py docstring must pin residual CI leftover surface "
                f"enum stub {stub}",
                errors,
            )
    residual_ci_leftover_host = "schema residual CI leftover deepen after " + "#299"
    self_text8 = BADGE_GATE.read_text(encoding="utf-8")
    if residual_ci_leftover_host not in self_text8:
        fail(
            "check_badge_standard.py must keep " + residual_ci_leftover_host + " host pin",
            errors,
        )
    residual_ci_leftover_slice = "schema residual CI leftover " + "slice"
    if residual_ci_leftover_slice not in self_text8:
        fail(
            "check_badge_standard.py schema residual CI leftover must keep "
            + residual_ci_leftover_slice
            + " pin",
            errors,
        )
    contract_residual_ci_leftover = "residual CI leftover deepen after " + "#299"
    if contract_residual_ci_leftover not in self_text8:
        fail(
            "check_stewardship_schema_gate_contract docstring must pin "
            + contract_residual_ci_leftover,
            errors,
        )
...
# Fail-closed after #326 tip-relaunch on post-#348: schema residual helper / constant / needle pins
    # (schema residual slice only; not Pass-2 residual templates leftovers #337 /
    # not wiki/mdlink leftover residual #326 /
    ...
    # not schema residual (pass-5) #233 spam).
    # Split literals so self-mutation of contiguous names cannot neutralize checks.
    residual_326_doc = "Residual deepen after " + "#326 (schema residual"
    if residual_326_doc not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + residual_326_doc,
            errors,
        )
    not_wiki_326 = "NOT wiki/mdlink leftover residual " + "#326"
    if not_wiki_326 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_wiki_326,
            errors,
        )
    not_schema_residual_ci_leftover_residual_320 = "NOT schema residual CI leftover residual " + "#320"
    if not_schema_residual_ci_leftover_residual_320 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_schema_residual_ci_leftover_residual_320,
            errors,
        )
    not_schema_residual_ci_leftover_309_c = "NOT schema residual CI leftover " + "#309"
    if not_schema_residual_ci_leftover_309_c not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_schema_residual_ci_leftover_309_c,
            errors,
        )
    not_schema_residual_ci_299_c = "NOT schema residual CI " + "#299"
    if not_schema_residual_ci_299_c not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_schema_residual_ci_299_c,
            errors,
        )
    not_wiki_293_leftover_c = "NOT wiki/mdlink leftover residual " + "#293"
    if not_wiki_293_leftover_c not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_wiki_293_leftover_c,
            errors,
        )
    not_schema_leftover_residual_282_c = "NOT schema leftover residual " + "#282"
    if not_schema_leftover_residual_282_c not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_schema_leftover_residual_282_c,
            errors,
        )
    not_pass2_337 = "NOT Pass-2 residual templates leftovers " + "#337"
    if not_pass2_337 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_pass2_337,
            errors,
        )
    not_pass2_348 = "NOT Pass-2 residual / templates leftovers after #337 " + "#348"
    if not_pass2_348 not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + not_pass2_348,
            errors,
        )
    fail_rel_exc = 'fail(f"{rel}: {exc}"'
    if fail_rel_exc not in text and "fail(f'{rel}: {exc}'" not in text:
        fail(
            'check_stewardship_schema.py must keep fail(f"{rel}: {exc}"',
            errors,
        )
    missing_meta_keys = "missing metadata keys"
    if missing_meta_keys not in text:
        fail(
            "check_stewardship_schema.py must keep missing metadata keys",
            errors,
        )
    fenced_yaml_search = "FENCED_YAML_RE.search"
    if fenced_yaml_search not in text:
        fail(
            "check_stewardship_schema.py must keep FENCED_YAML_RE.search",
            errors,
        )
    got_nested_list = "got nested/list"
    if got_nested_list not in text:
        fail(
            "check_stewardship_schema.py must keep got nested/list",
            errors,
        )
    type_value_name = "type(value).__name__"
    if type_value_name not in text:
        fail(
            "check_stewardship_schema.py must keep type(value).__name__",
            errors,
        )
    autonomy_int_0_3 = "autonomy_level must be int in 0..3"
    if autonomy_int_0_3 not in text:
        fail(
            "check_stewardship_schema.py must keep autonomy_level must be int in 0..3",
            errors,
        )
    tier_positive_int = "tier must be a positive int"
    if tier_positive_int not in text:
        fail(
            "check_stewardship_schema.py must keep tier must be a positive int",
            errors,
        )
    iso_date_prefixed = "must be ISO-8601 date-prefixed"
    if iso_date_prefixed not in text:
        fail(
            "check_stewardship_schema.py must keep must be ISO-8601 date-prefixed",
            errors,
        )
    no_invent_wording = "no-invent-product wording"
    if no_invent_wording not in text:
        fail(
            "check_stewardship_schema.py must keep no-invent-product wording",
            errors,
        )
    must_semver = "must be semver X.Y.Z"
    if must_semver not in text:
        fail(
            "check_stewardship_schema.py must keep must be semver X.Y.Z",
            errors,
        )
    must_issue_ref = "must reference an issue like #N"
    if must_issue_ref not in text:
        fail(
            "check_stewardship_schema.py must keep must reference an issue like #N",
            errors,
        )
    ok_schemas_valid = "OK: stewardship metadata schemas valid"
    if ok_schemas_valid not in text:
        fail(
            "check_stewardship_schema.py must keep OK: stewardship metadata schemas valid",
            errors,
        )
    len_doc_schemas = "len(DOC_SCHEMAS)"
    if len_doc_schemas not in text:
        fail(
            "check_stewardship_schema.py must keep len(DOC_SCHEMAS)",
            errors,
        )
    empty_yaml_block = "empty yaml metadata block"
    if empty_yaml_block not in text:
        fail(
            "check_stewardship_schema.py must keep empty yaml metadata block",
            errors,
        )
    parse_simple = "parse_simple_yaml"
    if parse_simple not in text:
        fail(
            "check_stewardship_schema.py must keep parse_simple_yaml",
            errors,
        )
    first_yaml = "first_yaml_block"
    if first_yaml not in text:
        fail(
            "check_stewardship_schema.py must keep first_yaml_block",
            errors,
        )
    unsupported_yaml = "unsupported YAML line"
    if unsupported_yaml not in text:
        fail(
            "check_stewardship_schema.py must keep unsupported YAML line",
            errors,
        )
    for stub in (
        "VOID",
    ...
    residual_326_host = "schema residual deepen after " + "#326"
    self_text10 = BADGE_GATE.read_text(encoding="utf-8")
    if residual_326_host not in self_text10:
        fail(
            "check_badge_standard.py must keep " + residual_326_host + " host pin",
            errors,
        )
    residual_326_slice = "schema residual " + "slice"
    if residual_326_slice not in self_text10:
        fail(
            "check_badge_standard.py schema residual must keep "
            + residual_326_slice
            + " pin",
    ...
    contract_residual_326 = "residual deepen after " + "#326"
    if contract_residual_326 not in self_text10:
        fail(
            "check_stewardship_schema_gate_contract docstring must pin "
            + contract_residual_326,
            errors,
        )
...
def check_wiki_outline_gate_contract(errors: list[str]) -> None:
    ...
    leftover_320_wiki = "Wiki/mdlink leftover residual after " + "#320"
    if leftover_320_wiki not in wiki_text:
        fail(
            "check_wiki_outline.py docstring must keep " + leftover_320_wiki + " pin",
            errors,
        )
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    if not RUN_STEWARDSHIP.is_file():
        ...
        # Pass-2 residual / templates leftover after #337 tip (soft-fail
        # leftovers beyond saturated #326 || true / set +e / eval /
        # NOT Pass-2 residual / templates leftover after #326 /
        # NOT lychee/blob-503 leftover #278 /
        # NOT wiki/mdlink leftover residual #326 /
        # NOT path-filter/path-order residual leftover #314 /
        # NOT stewardship-schema residual CI leftover residual #320).
        if "|| /bin/true" in run_text or "||/bin/true" in run_text:
            fail(
                "run_stewardship_checks.sh must not soft-fail with || /bin/true "
                "(Pass-2 residual / templates leftover after #337 tip)",
                errors,
            )
        if "|| /usr/bin/true" in run_text or "||/usr/bin/true" in run_text:
            fail(
                "run_stewardship_checks.sh must not soft-fail with || /usr/bin/true "
                "(Pass-2 residual / templates leftover after #337 tip)",
                errors,
            )
        if re.search(r"(?m)^\s*set \+E\b", run_text):
    ...
    leftover_320_rel = "Wiki/mdlink leftover residual after " + "#320"
...
def check_run_stewardship_gate_contract(errors: list[str]) -> None:
    ...
    # Pass-2 residual / templates leftover after #337 tip contract pins.
    leftover_337 = (
        "Pass-2 residual / templates leftover after " + "#337 tip"
    )
    if leftover_337 not in text:
        fail(
            "run_stewardship pins must keep " + leftover_337 + " marker",
            errors,
        )
    not_pass2_326 = "NOT Pass-2 residual / templates leftover after " + "#326"
    if not_pass2_326 not in text:
        fail(
            "run_stewardship docstring must keep " + not_pass2_326 + " distinctness",
            errors,
        )
    bin_true_pin = "soft-fail with || /bin/" + "true"
    if bin_true_pin not in text:
        fail(
            "run_stewardship contract must keep " + bin_true_pin + " pin",
            errors,
        )
    usr_true_pin = "soft-fail with || /usr/bin/" + "true"
    if usr_true_pin not in text:
        fail(
            "run_stewardship contract must keep " + usr_true_pin + " pin",
            errors,
        )
    set_E_pin = "soft-fail with set " + "+E"
    if set_E_pin not in text:
    ...
    section_pin = "[Section " + "Title]"
    if section_pin not in text:
        fail(
            "run_stewardship contract must keep " + section_pin + " leftover",
            errors,
        )
    version_pin = "### Version " + "History"
    if version_pin not in text:
        fail(
            "run_stewardship contract must keep " + version_pin + " leftover",
            errors,
        )
    blob_master_pin = "must not invent blob/master " + "exclude"
    if blob_master_pin not in text:
        fail(
            "run_stewardship contract must keep " + blob_master_pin + " pin",
            errors,
        )
    tree_master_pin = "must not invent tree/master " + "exclude"
    if tree_master_pin not in text:
        fail(
            "run_stewardship contract must keep " + tree_master_pin + " pin",
            errors,
        )
    raw_gh_pin = "must not invent raw.githubusercontent.com " + "exclude"
    if raw_gh_pin not in text:
        fail(
