...
Fail-closed actionlint-style pins (live path after #59):
...
def check_actionlint_style(errors: list[str]) -> None:
    ...
    Live fail-closed pins after #59 (not relative-link / schema-scalar spam):
    top-level name: / jobs.*.runs-on / jobs.*.steps / timeout-minutes;
    ...
    for name in REQUIRED_WORKFLOWS:
        ...
        # Fail-closed after #59: OIDC write not needed for docs CI paths.
...
def check_actionlint_style_gate_contract(errors: list[str]) -> None:
    """Fail-close live actionlint-style gate wiring (after #59; not relative-link spam)."""
    if not BADGE_GATE.is_file():
    ...
    live_pin = "Live fail-closed pins after " + "#59"
    ...
    # Slice marker so self-tests can assert this is not relative-link spam.
    if "not relative-link spam" not in text:
        fail(
            "check_actionlint_style_gate_contract must say not relative-link spam",
            errors,
        )
...
def main() -> int:
    ...
    # Fail-closed after #59: actionlint-style contract early (same self-host file)
