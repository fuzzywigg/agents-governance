...
Fail-closed Pass-2 residual / templates leftover after #293 tip
...
def check_lycheeignore(errors: list[str]) -> None:
    ...
    # Pass-2 residual / templates leftover after #293 tip (existing lychee
    # module only; NOT saturated lychee/blob-503 leftover #278 /
    # NOT saturated Pass-2 residual leftover #272 /
    # NOT wiki/mdlink leftover residual #293 / do not invent templates).
    if "tree/main" in text:
        fail(
            ".lycheeignore must not invent tree/main exclude "
            "(Pass-2 residual / templates leftover after #293 tip; "
...
def check_agents_repo_template(errors: list[str]) -> None:
    ...
    for needle, label in leftover_293_required:
        if needle not in text:
            fail(
                f"templates/AGENTS-REPO.md must keep {label} "
                "(Pass-2 residual / templates leftover after #293 tip)",
...
def check_run_stewardship_gate_contract(errors: list[str]) -> None:
    ...
    # Pass-2 residual / templates leftover after #293 tip contract pins.
    leftover_293 = (
        "Pass-2 residual / templates leftover after " + "#293 tip"
    )
    if leftover_293 not in text:
        fail(
            "run_stewardship pins must keep " + leftover_293 + " marker",
            errors,
        )
