...
Fail-closed wiki/mdlink leftover residual after #309 tip (beyond #293 saturated
after_293 pins; lands #318/#308/#307/#302 leftover (tip-relaunch after #318 DIRTY lint FAIL);
...
# Fail-closed after #227: markdown-lint/link-check residual exact layouts
    ...
    # Wiki/mdlink leftover residual after #309 tip: md/link residual harden beyond #293 (lands #318/#308/#307/#302).
    link_token_args_adj = (
        "          token: ${{ secrets.GITHUB_TOKEN }}\n"
        "          args: >-"
    )
    if link_token_args_adj not in link:
        fail(
            ...
            "(wiki/mdlink leftover residual after #309)",
            errors,
        )
    link_steps_triple = (
        "      - uses: actions/checkout@v7\n"
    ...
    if lint_steps_triple not in lint:
        fail(
            ...
            "triple adjacency (wiki/mdlink leftover residual after #309)",
            errors,
        )
...
def check_badge_standard_gate_contract(errors: list[str]) -> None:
    ...
    leftover_309_doc = "wiki/mdlink leftover residual after " + "#309"
    if leftover_309_doc not in text:
        fail(
            "check_badge_standard.py docstring must keep " + leftover_309_doc + " pin",
            errors,
        )
...
def check_wiki_outline_gate_contract(errors: list[str]) -> None:
    ...
    leftover_309_wiki = "Wiki/mdlink leftover residual after " + "#309"
    if leftover_309_wiki not in wiki_text:
        fail(
            "check_wiki_outline.py docstring must keep " + leftover_309_wiki + " pin",
            errors,
        )
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    leftover_309_rel = "Wiki/mdlink leftover residual after " + "#309"
    if leftover_309_rel not in text:
        fail(
            "check_relative_links.py docstring must keep " + leftover_309_rel + " pin",
            errors,
        )
