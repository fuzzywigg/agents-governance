...
Fail-closed actionlint path-order residual leftover deepen after #314
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    contiguous six-step actionlint path-order.
    Path-order residual leftover deepen after #314 (NOT saturated leftover
    ...
    # Path-order residual leftover deepen after #314 (DISTINCT leftover;
    # NOT saturated leftover #314 / NOT schema residual CI leftover #309 /
    # NOT schema residual CI #299 / NOT wiki/mdlink leftover residual #293 /
    # NOT stewardship-schema residual #282 /
    # NOT lychee/blob-503 harden #278 / NOT Pass-2 residual leftover #272 /
    ...
    # NOT md/link residual #239 / NOT stewardship-checks/schema leftover #233 /
    # NOT wiki-badge leftover #227 / NOT stewardship-badge lint #208).
    checkout_idx_314 = stew.find("uses: actions/checkout@v7")
    setup_idx_314 = stew.find("name: Set up Python")
    install_idx_314 = stew.find("name: Install PyYAML (schema parser)")
    gates_name_314 = (
        "name: Stewardship gates (badge / wiki / schema / relative links)"
    )
    gates_idx_314 = stew.find(gates_name_314)
    self_tests_idx_314 = stew.find("name: Stewardship gate self-tests")
    dl_idx_314 = stew.find("name: Download actionlint")
    run_idx_314 = stew.find("name: actionlint existing workflow paths")
    if (
        checkout_idx_314 < 0
        or setup_idx_314 < 0
        or install_idx_314 < 0
        or gates_idx_314 < 0
        or self_tests_idx_314 < 0
        or dl_idx_314 < 0
        or run_idx_314 < 0
        or not (
            checkout_idx_314
...
def check_workflow_hardening_gate_contract(errors: list[str]) -> None:
    ...
    # Path-order residual leftover deepen after #314 (DISTINCT leftover).
    path_leftover_314_doc = (
        "Path-order residual leftover deepen after " + "#314"
    )
    if path_leftover_314_doc not in text:
        fail(
            "check_workflow_hardening docstring must pin " + path_leftover_314_doc,
            errors,
        )
    path_module_leftover_314 = (
        "path-order residual leftover deepen after " + "#314"
    )
    if path_module_leftover_314 not in text:
        fail(
            "check_badge_standard.py module docstring must pin "
            + path_module_leftover_314,
            errors,
    ...
    if not_saturated_leftover_314 not in text:
        fail(
            "path leftover-after-314 must keep "
    ...
    for needle, label in path_leftover_314_pins:
        if needle not in text:
            fail(
                "check_workflow_hardening must keep " + label,
                errors,
