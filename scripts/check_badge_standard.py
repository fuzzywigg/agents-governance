...
Fail-closed actionlint-style pins (live path after #72):
...
def check_actionlint_style(errors: list[str]) -> None:
    ...
    Live fail-closed pins after #72 (not badge-row / relative-pin spam):
    top-level name: / jobs.*.runs-on / jobs.*.steps / timeout-minutes;
    ...
    for name in REQUIRED_WORKFLOWS:
        ...
        # Fail-closed after #72: OIDC write not needed for docs CI paths.
...
def check_actionlint_style_gate_contract(errors: list[str]) -> None:
    """Fail-close live actionlint-style gate wiring (after #72; not relative-pin spam)."""
    if not BADGE_GATE.is_file():
    ...
    live_pin = "Live fail-closed pins after " + "#72"
...
def main() -> int:
    ...
    # Fail-closed after #72: actionlint-style contract early (same self-host file)
