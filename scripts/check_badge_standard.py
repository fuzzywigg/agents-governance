...
Fail-closed stewardship-badge lint deepen after #161 (NOT docs-lint leftover
#161 / NOT wiki-badge #141; residual uncovered only):
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    # Stewardship-badge lint deepen after #161: exact live markdown-lint globs
    # block (path globs already coded; residual exact layout vs docs-lint #161).
    expected_globs = (
        "          globs: |\n"
        "            **/*.md\n"
        "            !.github/agents/**\n"
    ...
    if expected_globs not in lint:
        fail(
            "markdown-lint.yml must match exact live stewardship-badge lint globs block after #161",
            errors,
        )
    # Exact live markdown-lint push paths filter (path globs already coded).
    expected_md_paths = (
        '    paths:\n'
        '      - "**/*.md"\n'
        '      - ".markdownlint.json"\n'
    ...
    if expected_md_paths not in lint:
        fail(
            "markdown-lint.yml must match exact live stewardship-badge lint push paths filter after #161",
            errors,
        )
    # Exact live link-check push paths filter (badge-adjacent path globs).
    expected_link_paths = (
        '    paths:\n'
        '      - "**/*.md"\n'
        '      - ".lycheeignore"\n'
    ...
    if expected_link_paths not in link:
        fail(
            "link-check.yml must match exact live stewardship-badge lint push paths filter after #161",
            errors,
        )
    # Path-filter membership affirm for markdownlint config (complement membership).
    if '".markdownlint.json"' not in lint and "'.markdownlint.json'" not in lint:
        fail(
            ...
            "(stewardship-badge lint after #161)",
            errors,
        )
...
def check_readme_consistency(text: str, errors: list[str]) -> None:
    ...
    # Stewardship-badge lint deepen after #161: no invent workflow badge.svg
    # (residual vs wiki-badge #141 which covers wiki pages only).
    if "stewardship-checks.yml/badge.svg" in text:
        fail(
            "README.md must not invent stewardship-checks.yml/badge.svg "
            "(no fourth badge; stewardship-badge lint after #161)",
            errors,
        )
...
def check_badge_standard_gate_contract(errors: list[str]) -> None:
    ...
    # Stewardship-badge lint deepen after #161 (NOT docs-lint leftover /
    # NOT wiki-badge; residual README invent + exact badge.svg paths).
    after_161 = "after " + "#161"
    if after_161 not in text:
        fail(
            "check_badge_standard.py must pin stewardship-badge lint " + after_161,
            errors,
        )
...
def check_workflow_hardening_gate_contract(errors: list[str]) -> None:
    ...
    # Stewardship-badge lint deepen after #161: exact globs + paths contract.
    after_161_wf = "after " + "#161"
    if after_161_wf not in text:
        fail(
            "workflow hardening must pin stewardship-badge lint " + after_161_wf,
            errors,
        )
    ...
    not_docs_161 = "NOT docs-lint " + "leftover"
    if not_docs_161 not in text:
        fail(
            "stewardship-badge lint must keep " + not_docs_161 + " distinctness",
            errors,
        )
