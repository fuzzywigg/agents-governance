...
- schema residual CI deepen after #282: residual CI helper needles + residual CI status/surface stubs (schema residual CI slice)
  (NOT schema leftover residual #282 / NOT lychee/blob-503 leftover #278 / NOT Pass-2 residual leftover #272 / NOT path-filter/path-order leftover #262 / NOT schema leftover #258 / NOT wiki/mdlink leftover #252 / NOT path-edges residual #244 / NOT schema residual (pass-5) #233)
...
def check_stewardship_common_contract(errors: list[str]) -> None:
    """Fail-close live stewardship_common wiring (after #111; deepen after #65/#46; leftover after #252; leftover residual after #262; residual CI deepen after #282)."""
...
def check_stewardship_schema_gate_contract(errors: list[str]) -> None:
    """Fail-close live stewardship-schema gate wiring (after #72/#75; third-pass after #132; deepen after #189; policy-schema pass-4 after #208; schema residual after #225; leftover after #252; leftover residual after #262; residual CI deepen after #282)."""
    ...
    # Fail-closed after #282: schema residual CI helper / constant / needle pins
    # (schema residual CI slice only; not schema leftover residual #282 /
    ...
    # not schema residual (pass-5) #233 spam).
    # Split literals so self-mutation of contiguous names cannot neutralize checks.
    residual_ci_282_doc = "Residual CI deepen after " + "#282 (schema residual CI"
    if residual_ci_282_doc not in text:
        fail(
            "check_stewardship_schema.py docstring must pin " + residual_ci_282_doc,
            errors,
        )
    ...
    residual_ci_host = "schema residual CI deepen after " + "#282"
    self_text6 = BADGE_GATE.read_text(encoding="utf-8")
    if residual_ci_host not in self_text6:
        fail(
            "check_badge_standard.py must keep " + residual_ci_host + " host pin",
            errors,
        )
    residual_ci_slice = "schema residual CI " + "slice"
    if residual_ci_slice not in self_text6:
        fail(
    ...
    contract_residual_ci = "residual CI deepen after " + "#282"
    if contract_residual_ci not in self_text6:
        fail(
            "check_stewardship_schema_gate_contract docstring must pin "
