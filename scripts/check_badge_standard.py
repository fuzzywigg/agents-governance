...
Fail-closed leftover docs-lint/stewardship/actionlint pins after #141:
...
- actionlint leftover: contents: read / cancel-in-progress: true /
  ...
  statuses|deployments: write (affirm least-privilege runner posture)
...
def check_lycheeignore(errors: list[str]) -> None:
    ...
    Third-pass after #141: exact live .lycheeignore full layout + exact
    ...
    # Third-pass after #141: exact live .lycheeignore full layout (docs-lint leftover).
    expected_lychee = (
    ...
    # Third-pass after #141: exact live commentary line pins (docs-lint leftover).
    mcp_line = (
...
def check_actionlint_style(errors: list[str]) -> None:
    ...
    Leftover after #141: contents: read / cancel-in-progress: true /
    ubuntu-latest / workflow_dispatch: / reject security-events|attestations|
    statuses|deployments: write (actionlint leftover; not docs-lint / wiki spam).
    """
    for name in REQUIRED_WORKFLOWS:
...
def check_docs_lint_gate_contract(errors: list[str]) -> None:
    ...
    # Third-pass after #141: docs-lint leftover exact layout + commentary pins.
    third_pass = "Third-pass after " + "#141"
...
def check_actionlint_style_gate_contract(errors: list[str]) -> None:
    ...
    # Leftover after #141: actionlint contents:read / cancel true / ubuntu pins.
    leftover_141 = "Leftover after " + "#141"
    if leftover_141 not in text:
        fail(
            "check_actionlint_style docstring must pin " + leftover_141,
            errors,
        )
    ...
    wd_req = "actionlint-style requires workflow_" + "dispatch:"
    if wd_req not in text:
        fail(
            "check_actionlint_style must emit " + wd_req + " fail needle",
            errors,
        )
    ...
    if r"cancel-in-progress:\s*true" not in text:
        fail(
    ...
    sec_write = "security-events: write is " + "forbidden"
    if sec_write not in text:
        fail(
            "check_actionlint_style must emit " + sec_write + " fail needle",
            errors,
        )
    att_write = "attestations: write is " + "forbidden"
    if att_write not in text:
        fail(
            "check_actionlint_style must emit " + att_write + " fail needle",
            errors,
        )
    st_write = "statuses: write is " + "forbidden"
    if st_write not in text:
        fail(
            "check_actionlint_style must emit " + st_write + " fail needle",
            errors,
        )
    dep_write = "deployments: write is " + "forbidden"
    if dep_write not in text:
        fail(
            "check_actionlint_style must emit " + dep_write + " fail needle",
            errors,
        )
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    if not RUN_STEWARDSHIP.is_file():
        ...
        # Leftover after #141: exact live run_stewardship_checks.sh full layout.
        expected_run = (
            "#!/usr/bin/env bash\n"
...
def check_run_stewardship_gate_contract(errors: list[str]) -> None:
    ...
    # Leftover after #141: exact run_stewardship layout contract pins.
    leftover_141 = "Leftover after " + "#141"
    if leftover_141 not in text:
        fail(
            "run_stewardship pins must keep " + leftover_141 + " marker",
            errors,
        )
    ...
    leftover_after = "stewardship leftover after " + "#141"
