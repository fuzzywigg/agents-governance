...
after #227 tip (NOT Pass-2 residual #199/#203 / NOT Pass-2 leftover + md/link #220 /
NOT path-edges #225 / NOT wiki-index/badge #227 / NOT badge-lint #208 / NOT schema #216 /
NOT path-order #189 / NOT path-filter #176):
...
def check_workflow_hardening(errors: list[str]) -> None:
    ...
    must not set continue-on-error / exact job permissions: contents: read /
    checkout before Check links adjacency / checkout before Run markdownlint adjacency.
    Pass-2 residual deepen + concurrency template validation residual after #227 tip
    ...
    for name in REQUIRED_WORKFLOWS:
        ...
        # NOT path-edges #225 / NOT wiki-index/badge #227 / NOT Pass-2 leftover #220).
    ...
    # beyond path-order true-only pin (NOT path-edges #225 / NOT wiki-index/badge #227 /
...
def check_relative_link_gate_contract(errors: list[str]) -> None:
    ...
    if not RUN_STEWARDSHIP.is_file():
        ...
        # NOT wiki-index/badge #227 / NOT md/link #220 / NOT badge-lint #208).
...
def check_run_stewardship_gate_contract(errors: list[str]) -> None:
    ...
    not_wiki_227 = "NOT wiki-index/badge " + "#227"
    if not_wiki_227 not in text:
        fail(
            "run_stewardship docstring must keep " + not_wiki_227 + " distinctness",
            errors,
        )
