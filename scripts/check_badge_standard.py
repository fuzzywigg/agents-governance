...
Fail-closed leftover docs-lint/stewardship/actionlint pins after #135/#141:
...
def check_actionlint_style(errors: list[str]) -> None:
    ...
    for name in REQUIRED_WORKFLOWS:
        ...
        # Leftover after #135: affirm contents: read (least privilege).
        if "contents: read" not in text:
            fail(f"{name}: actionlint-style requires contents: read", errors)
        # Leftover after #135: cancel-in-progress must be true (not bare / false).
