...
- After #108 wiki-badge posture: status badges cover Link Check+Markdown Lint /
...
def main() -> int:
    ...
    if publish.is_file():
        ...
        # Fail-closed after #108: exact badge-name phrases (wiki-badge posture).
    ...
    if home.is_file():
        ...
        # Fail-closed after #108: Home is narrative — no badge-row embeds.
        if "[![" in home_text:
            fail(
    ...
    if stewardship.is_file():
        ...
        # Fail-closed after #108: wiki-badge posture (Link Check + Markdown Lint only).
        if "Link Check" not in ste_text or "Markdown Lint" not in ste_text:
            fail(
                "Repo-Stewardship.md must name Link Check and Markdown Lint "
                "status badges",
                errors,
            )
    ...
    for name in PUBLISHABLE_PAGES:
        ...
        # Fail-closed after #108: no fourth-badge invent / no badge-row embeds on wiki.
