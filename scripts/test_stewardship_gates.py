...
CI workflow pin spam).
Deepened after #53: actionlint-style gate contract pins — top-level
...
(not badge / common / schema / wiki / relative / CI workflow pin spam).
"""
...
# --- TOKENMAXX deepen after #53: actionlint-style gate contract / fail-closed pins ---
...
def test_actionlint_style_gate_requires_fn_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_static_doc_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _mutate_badge_gate(
            tmp_path, "Static actionlint-like checks", "Static actionlint soft checks"
        )
        assert_fail_script(scripts / "check_badge_standard.py", tmp_path, "Static actionlint-like checks")


def test_actionlint_style_gate_requires_live_pins_doc_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _mutate_badge_gate(
            tmp_path, "Live fail-closed pins after #53", "Live fail-closed pins after #00"
        )
        assert_fail_script(scripts / "check_badge_standard.py", tmp_path, "Live fail-closed pins after #53")


def test_actionlint_style_gate_requires_load_workflow_text_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_top_level_name_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_jobs_runs_on_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_jobs_steps_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_timeout_minutes_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_prt_reject_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_harden_wording_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_write_all_reject_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_contents_write_reject_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_id_token_write_reject_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_docker_skip_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_unpinned_reject_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
...
def test_actionlint_style_gate_requires_float_main_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _mutate_badge_gate(
...
def test_actionlint_style_gate_requires_float_master_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _mutate_badge_gate(
...
def test_actionlint_style_gate_requires_float_latest_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _mutate_badge_gate(
...
def test_actionlint_style_gate_requires_float_set_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _mutate_badge_gate(
...
def test_actionlint_style_gate_requires_main_call_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _mutate_badge_gate(
            tmp_path, "check_actionlint_style(errors)", "check_actionlint_ok(errors)"
        )
        assert_fail_script(scripts / "check_badge_standard.py", tmp_path, "check_actionlint_style(errors)")


def test_actionlint_style_rejects_id_token_write_link_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
...
def test_actionlint_style_rejects_id_token_write_markdown_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
...
def test_actionlint_style_rejects_id_token_write_stewardship_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
...
def test_actionlint_style_rejects_missing_name_link_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
        wf = tmp_path / ".github" / "workflows" / "link-check.yml"
...
def test_actionlint_style_rejects_missing_runs_on_markdown_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
        wf = tmp_path / ".github" / "workflows" / "markdown-lint.yml"
...
def test_actionlint_style_rejects_missing_steps_stewardship_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
        wf = tmp_path / ".github" / "workflows" / "stewardship-checks.yml"
...
def test_actionlint_style_rejects_missing_timeout_link_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
        wf = tmp_path / ".github" / "workflows" / "link-check.yml"
...
def test_actionlint_style_rejects_prt_markdown_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
        wf = tmp_path / ".github" / "workflows" / "markdown-lint.yml"
...
def test_actionlint_style_rejects_write_all_stewardship_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
        wf = tmp_path / ".github" / "workflows" / "stewardship-checks.yml"
...
def test_actionlint_style_rejects_contents_write_link_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
        wf = tmp_path / ".github" / "workflows" / "link-check.yml"
...
def test_actionlint_style_rejects_unpinned_checkout_markdown_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
        wf = tmp_path / ".github" / "workflows" / "markdown-lint.yml"
...
def test_actionlint_style_rejects_float_main_stewardship_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
        wf = tmp_path / ".github" / "workflows" / "stewardship-checks.yml"
        text = wf.read_text(encoding="utf-8").replace("actions/checkout@v7", "actions/checkout@main")
        wf.write_text(text, encoding="utf-8")
        assert_fail_script(scripts / "check_badge_standard.py", tmp_path, "@main")


def test_actionlint_style_rejects_float_latest_link_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
        wf = tmp_path / ".github" / "workflows" / "link-check.yml"
...
def test_actionlint_style_rejects_float_master_markdown_after_53() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        scripts = _seed_badge_tree(tmp_path, _good_readme())
        wf = tmp_path / ".github" / "workflows" / "markdown-lint.yml"
...
def test_actionlint_style_gate_contract_fn_still_after_53() -> None:
    sys.path.insert(0, str(SCRIPTS))
    import importlib
    import check_badge_standard as cbs  # noqa: E402
    importlib.reload(cbs)
...
def test_actionlint_style_fn_still_after_53() -> None:
    sys.path.insert(0, str(SCRIPTS))
    import importlib
    import check_badge_standard as cbs  # noqa: E402
    importlib.reload(cbs)
...
def test_actionlint_style_gate_contract_errors_empty_live_still_after_53() -> None:
    sys.path.insert(0, str(SCRIPTS))
    import importlib
    import check_badge_standard as cbs  # noqa: E402
    importlib.reload(cbs)
...
def test_actionlint_style_behavioral_errors_empty_live_still_after_53() -> None:
    sys.path.insert(0, str(SCRIPTS))
    import importlib
    import check_badge_standard as cbs  # noqa: E402
    importlib.reload(cbs)
...
def test_actionlint_style_contract_before_behavioral_still_after_53() -> None:
    text = (SCRIPTS / "check_badge_standard.py").read_text(encoding="utf-8")
    c_i = text.find("check_actionlint_style_gate_contract(errors)")
    b_i = text.find("check_actionlint_style(errors)")
    # First behavioral call after early contract; contract must appear before first style call
    # that is NOT inside the contract function body string checks.
    # Use main() block: find "def main" then compare.
    ...
    if c_main < 0 or b_main < 0 or not (c_main < b_main):
        raise AssertionError("main must call actionlint-style contract before behavioral check")


def test_actionlint_style_load_workflow_text_still_after_53() -> None:
    text = (SCRIPTS / "check_badge_standard.py").read_text(encoding="utf-8")
    # Style function body must call load_workflow_text
...
def test_actionlint_style_required_workflows_loop_still_after_53() -> None:
    text = (SCRIPTS / "check_badge_standard.py").read_text(encoding="utf-8")
    start = text.find("def check_actionlint_style(")
    end = text.find("\ndef check_workflow_hardening(", start)
    body = text[start:end]
...
def test_actionlint_style_source_has_contract_section_still_after_53() -> None:
    text = (SCRIPTS / "check_badge_standard.py").read_text(encoding="utf-8")
...
def test_actionlint_style_contract_mentions_not_badge_spam_still_after_53() -> None:
    text = (SCRIPTS / "check_badge_standard.py").read_text(encoding="utf-8")
    start = text.find("def check_actionlint_style_gate_contract(")
    end = text.find("\ndef check_stewardship_common_contract(", start)
    body = text[start:end]
    if "not badge-pin spam" not in body:
        raise AssertionError("actionlint-style contract must say not badge-pin spam")


def test_actionlint_style_link_workflow_live_name_still_after_53() -> None:
...
def test_actionlint_style_markdown_workflow_live_runs_on_still_after_53() -> None:
...
def test_actionlint_style_stewardship_workflow_live_steps_still_after_53() -> None:
    text = (ROOT / ".github" / "workflows" / "stewardship-checks.yml").read_text(encoding="utf-8")
    if "steps:" not in text:
        raise AssertionError("stewardship-checks.yml missing steps:")


def test_actionlint_style_all_workflows_no_prt_still_after_53() -> None:
...
def test_actionlint_style_all_workflows_no_id_token_write_still_after_53() -> None:
    for name in ("link-check.yml", "markdown-lint.yml", "stewardship-checks.yml"):
        text = (ROOT / ".github" / "workflows" / name).read_text(encoding="utf-8")
...
def test_actionlint_style_all_workflows_no_write_all_still_after_53() -> None:
    for name in ("link-check.yml", "markdown-lint.yml", "stewardship-checks.yml"):
        text = (ROOT / ".github" / "workflows" / name).read_text(encoding="utf-8")
...
def test_actionlint_style_all_workflows_pinned_uses_still_after_53() -> None:
    import re
    for name in ("link-check.yml", "markdown-lint.yml", "stewardship-checks.yml"):
        text = (ROOT / ".github" / "workflows" / name).read_text(encoding="utf-8")
...
def test_actionlint_style_selftest_doc_mentions_after_53_still() -> None:
    text = (SCRIPTS / "test_stewardship_gates.py").read_text(encoding="utf-8")
    if "Deepened after #53: actionlint-style" not in text:
        raise AssertionError("self-test docstring must mention Deepened after #53")
...
def main() -> int:
    tests = [
        ...
        test_actionlint_style_gate_requires_fn_after_53,
        test_actionlint_style_gate_requires_static_doc_after_53,
        test_actionlint_style_gate_requires_live_pins_doc_after_53,
        test_actionlint_style_gate_requires_load_workflow_text_after_53,
        test_actionlint_style_gate_requires_top_level_name_after_53,
        test_actionlint_style_gate_requires_jobs_runs_on_after_53,
        test_actionlint_style_gate_requires_jobs_steps_after_53,
        test_actionlint_style_gate_requires_timeout_minutes_after_53,
        test_actionlint_style_gate_requires_prt_reject_after_53,
        test_actionlint_style_gate_requires_harden_wording_after_53,
        test_actionlint_style_gate_requires_write_all_reject_after_53,
        test_actionlint_style_gate_requires_contents_write_reject_after_53,
        test_actionlint_style_gate_requires_id_token_write_reject_after_53,
        test_actionlint_style_gate_requires_docker_skip_after_53,
        test_actionlint_style_gate_requires_unpinned_reject_after_53,
        test_actionlint_style_gate_requires_float_main_after_53,
        test_actionlint_style_gate_requires_float_master_after_53,
        test_actionlint_style_gate_requires_float_latest_after_53,
        test_actionlint_style_gate_requires_float_set_after_53,
        test_actionlint_style_gate_requires_main_call_after_53,
        test_actionlint_style_rejects_id_token_write_link_after_53,
        test_actionlint_style_rejects_id_token_write_markdown_after_53,
        test_actionlint_style_rejects_id_token_write_stewardship_after_53,
        test_actionlint_style_rejects_missing_name_link_after_53,
        test_actionlint_style_rejects_missing_runs_on_markdown_after_53,
        test_actionlint_style_rejects_missing_steps_stewardship_after_53,
        test_actionlint_style_rejects_missing_timeout_link_after_53,
        test_actionlint_style_rejects_prt_markdown_after_53,
        test_actionlint_style_rejects_write_all_stewardship_after_53,
        test_actionlint_style_rejects_contents_write_link_after_53,
        test_actionlint_style_rejects_unpinned_checkout_markdown_after_53,
        test_actionlint_style_rejects_float_main_stewardship_after_53,
        test_actionlint_style_rejects_float_latest_link_after_53,
        test_actionlint_style_rejects_float_master_markdown_after_53,
        test_actionlint_style_gate_contract_fn_still_after_53,
        test_actionlint_style_fn_still_after_53,
        test_actionlint_style_gate_contract_errors_empty_live_still_after_53,
        test_actionlint_style_behavioral_errors_empty_live_still_after_53,
        test_actionlint_style_docstring_static_still_after_53,
        test_actionlint_style_docstring_live_pins_still_after_53,
        test_actionlint_style_module_doc_pins_still_after_53,
        test_actionlint_style_id_token_needle_still_after_53,
        test_actionlint_style_docker_skip_still_after_53,
        test_actionlint_style_unpinned_needle_still_after_53,
        test_actionlint_style_float_set_still_after_53,
        test_actionlint_style_prt_needle_still_after_53,
        test_actionlint_style_write_all_needle_still_after_53,
        test_actionlint_style_contents_write_needle_still_after_53,
        test_actionlint_style_runs_on_needle_still_after_53,
        test_actionlint_style_steps_needle_still_after_53,
        test_actionlint_style_timeout_needle_still_after_53,
        test_actionlint_style_harden_wording_still_after_53,
        test_actionlint_style_main_calls_style_still_after_53,
        test_actionlint_style_main_calls_contract_still_after_53,
        test_actionlint_style_contract_before_behavioral_still_after_53,
        test_actionlint_style_load_workflow_text_still_after_53,
        test_actionlint_style_required_workflows_loop_still_after_53,
        test_actionlint_style_source_has_contract_section_still_after_53,
        test_actionlint_style_contract_mentions_not_badge_spam_still_after_53,
        test_actionlint_style_link_workflow_live_name_still_after_53,
        test_actionlint_style_markdown_workflow_live_runs_on_still_after_53,
        test_actionlint_style_stewardship_workflow_live_steps_still_after_53,
        test_actionlint_style_all_workflows_no_prt_still_after_53,
        test_actionlint_style_all_workflows_no_id_token_write_still_after_53,
        test_actionlint_style_all_workflows_no_write_all_still_after_53,
        test_actionlint_style_all_workflows_pinned_uses_still_after_53,
        test_actionlint_style_selftest_doc_mentions_after_53_still,
